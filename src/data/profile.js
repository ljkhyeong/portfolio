export const portfolioProfile = {
    ...
    printSummary:
        "BEINTECH에서 KICS와 통신사 및 전자영장 집행포털 사이의 요청 및 제출 자료를 연계하는 인터페이스와 Spring Batch를 개발하고 있습니다. 군사법 시스템에서는 Jenkins 실행 이력, WAS 로그와 DB 상태를 대조해 연계 장애를 확인하고 재처리했습니다. 개인 프로젝트에서는 요청 ID와 DB 처리 상태로 중복 결제와 알림 유실을 막고, 재고 행을 잠가 동시 주문의 초과 차감을 방지했습니다.",
}
...
export const workPrinciples = [
    {
        ...
        description:
            "BATON에서는 조직 데이터와 사용자의 참여 가능 여부를 Core가 관리합니다. 링크 생성, URL 점검, 이벤트 전달, 주간 보고서, 캘린더 구독과 화상 회의는 각각 별도 서비스에서 처리합니다.",
        printDescription:
    ...
    },
    {
        ...
        description:
            "기술 선택 이유는 의사결정 기록(ADR)에 남기고, API 요청 및 응답은 REST Docs와 OpenAPI로 관리합니다. 통합 및 E2E 테스트로 결제, 주문과 예약 흐름이 변경 후에도 유지되는지 확인합니다.",
        printDescription:
...
export const education = {
    ...
    projectId: "webrtc",
    description:
        "mediasoup의 RTP 영상을 FFmpeg와 GStreamer로 HLS 세그먼트와 재생 목록으로 만드는 서버와, WebRTC 실시간 영상 및 HLS 지난 구간을 재생하는 React 화면을 구현했습니다. 세그먼트 길이와 FFmpeg 설정을 조정해 재생 지연을 약 35초에서 17초로 줄였습니다.",
}
