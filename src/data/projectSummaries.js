export const projectSummaries = [
    {
        ...
        eyebrow: "조직 운영 플랫폼",
        summary:
            "조직의 역할, 반복 업무, 의사결정과 인수인계를 한곳에서 관리하는 서비스입니다. 조직 데이터와 조직 작업 공간 접속용 공유 키는 중앙 서비스인 Core가 관리하고, 화상방 참여권을 발급할 때 계정과 연결된 활동 중인 스터디 구성원인지 확인합니다. 링크 생성, URL 점검, 서비스 간 이벤트 전달, 주간 보고서, 캘린더 구독과 화상 회의는 6개 마이크로서비스가 각각 처리합니다.",
        homeFacts: [
            {
                label: "담당",
                value: "조직 데이터, 조직 작업 공간 접속용 공유 키와 화상방 참여권을 담당하는 중앙 서비스 및 링크, URL 점검, 이벤트 전달, 보고서, 캘린더와 화상방 서비스의 서버 기능, 저장 방식, 자동화 테스트와 배포 절차 구현",
            },
            {
                label: "문제",
            ...
            {
                label: "해결",
                value: "GO는 같은 링크 요청에 기존 결과를 반환하고, RELAY는 같은 이벤트가 다시 들어와도 등록된 각 전송 대상의 새 전달 작업을 추가하지 않았습니다. 전송을 시작하기 전에 발생한 일시 실패만 다시 처리하고, 외부 전송 요청의 성공 여부를 확인할 수 없으면 자동 재시도를 중단했습니다.",
            },
        ],
    ...
    },
    {
        ...
        eyebrow: "BEINTECH / LG CNS 컨소시엄 / KICS-통신사 및 집행포털 연계",
        summary:
            "전자영장 자료 제공 요청과 금융기관 및 통신사의 제출 자료를 독립망 사이에서 전달하는 공공 시스템입니다. LG CNS 컨소시엄에서 해양경찰 KICS 요청을 통신사와 집행포털 규격으로 변환해 보내고, 연계를 통해 수신한 제출 자료를 다시 KICS에 반영하는 서버 기능과 Spring Batch를 개발하고 있습니다.",
        homeFacts: [
            {
                label: "담당",
                value: "KICS 요청을 통신사와 집행포털 규격으로 변환해 보내고, 제출 자료를 다시 KICS에 반영하는 서버 기능과 Spring Batch",
            },
            {
                label: "문제",
    ...
    },
    {
        ...
        summary:
            "공방 고객이 작품을 주문하고 클래스를 예약하며, 관리자가 상품, 재고, 일정과 주문 상태를 처리하는 서비스입니다. 결제 및 환불 요청 ID로 중복 처리를 막고, 결과를 확인할 수 없으면 결제사에서 기존 처리 결과를 다시 조회한 뒤 복구 배치로 재처리합니다. 옵션별 재고는 항상 같은 순서로 잠근 뒤 차감했으며 AWS 운영 이력이 있습니다.",
        homeFacts: [
            {
                label: "담당",
            ...
            {
                label: "문제",
                value: "결제 승인 결과 미수신, 주문 저장 직후 서버 중단으로 인한 알림 유실과 동시 주문 시 옵션 재고 초과 차감",
            },
            {
                label: "해결",
                value: "결제 요청 ID로 중복 승인을 막고, 미전송 알림은 DB에서 다시 처리하며, 옵션 재고 행은 항상 같은 순서로 잠근 뒤 차감",
            },
        ],
    ...
    },
    {
        ...
        summary:
            "SeungIl 님이 개발한 Hope 3.0.3을 포크해 Commit Diff를 추가했습니다. 사용자가 고른 커밋과 비교 대상으로 확정한 부모 커밋에 저장된 코드만 비교하고 결과를 HTML로 저장합니다. 현재 수정 파일과 이전 대화를 제외하며, 리뷰 설명이 실제 변경 파일과 줄을 가리키는지도 검증합니다.",
        homeFacts: [
            {
                label: "담당",
            ...
            {
                label: "해결",
                value: "입력 커밋과 비교 대상으로 확정한 부모 커밋에 저장된 파일만 수집하고, 설명이 실제 변경 줄을 가리킬 때만 새 HTML 생성",
            },
        ],
    ...
    },
    {
        ...
        summary:
            "군 사법 업무와 군교정시설 수용 대상자의 정보를 폐쇄망에서 처리하는 국방부 시스템입니다. 세 기관의 수용자 인적정보 및 영장정보를 검증해 군교정 DB에 반영하는 배치를 개발했습니다. WebSquare 업무 화면의 상태 변경 요청 위조 차단, 대용량 파일 직접 업로드와 운영 장애 재처리도 담당했습니다.",
        homeFacts: [
            {
                label: "담당",
                value: "군교정 업무 화면과 수용자 인적정보 및 영장정보 검증 배치",
            },
            {
                label: "문제",
                value: "기관마다 다른 수용자 자료 형식, 위조된 상태 변경 요청과 대용량 파일 업로드 시 업무 서버 부하",
            },
            {
                label: "해결",
