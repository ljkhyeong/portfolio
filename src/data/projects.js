...
const projects = [
    {
        ...
        architecture: {
            label: "서비스 구성과 담당 업무",
            title: "Core는 조직 정보와 접근 권한을 관리하고, 링크 생성, URL 점검, 알림 전송, 주간 보고서, 캘린더 구독과 실시간 스터디룸은 6개 서비스가 나눠 처리합니다.",
            description:
                "Core는 팀, 시즌, 역할, 반복 업무, 의사결정과 인수인계를 저장하고 사용자가 스터디룸에 들어갈 수 있는지 확인합니다. GO는 짧은 링크, WATCH는 외부 URL 점검, RELAY는 알림 전송, BRIEF는 현재 확인 목록과 주간 보고서, CAL은 iCalendar 구독 피드, ROUND는 최대 6명의 WebRTC 연결을 각각 별도 저장소와 실행 환경에서 처리합니다.",
            tradeoff:
        ...
        services: [
            {
                ...
                evidence: "PRD 6 · ADR 19 · OpenAPI · 서비스 간 이벤트 및 참여권 규격",
            ...
            },
            {
                ...
                contribution:
                    "링크 생성과 조회, 활성 시작일 및 만료일, 폐기와 리다이렉트, UUID 멱등 처리와 HMAC 키 검증을 설계하고 구현했습니다.",
                stack: [
                    "Java 21",
                    "Spring Boot 4.1",
                    "Spring MVC",
                ...
                detail: "UUID 멱등 키, HMAC-SHA256 링크 코드와 허용 대상 타입 및 경로 검증",
                evidence:
            ...
            },
            {
                ...
                contribution:
                    "URL 입력값 검증, 한 서버가 점검 작업을 가져가는 처리와 상태 변경 이벤트를 DB에 저장한 뒤 재전송하는 기능을 구현했습니다.",
                stack: [
                    "Java 21",
                    "Spring Boot 4.1",
                    "Spring MVC",
                ...
                detail: "SSRF 방어, 한 서버가 가져간 작업의 만료 후 재처리, 이전 점검 결과의 덮어쓰기 차단과 미전송 이벤트 보관",
            ...
            },
            {
                ...
                contribution:
                    "eventId를 Inbox에 저장해 같은 이벤트를 두 번 처리하지 않도록 했습니다. 외부 호출 전에 전송 시도와 공급자용 멱등 키를 저장하고 RabbitMQ 수신과 SQS FIFO 전송을 구현했습니다.",
                stack: [
                    "Java 21",
                    "Spring Boot 4.1",
                    "Spring MVC",
                ...
                detail: "eventId 기반 중복 차단, 처리할 작업 선점, 제한 횟수 재시도와 성공 여부를 모르는 전송의 별도 보관",
                evidence:
                    "같은 메시지 재전달 시 중복 발송을 막고 작업 중단 뒤에도 같은 전송 식별자로 이어서 처리하는지 검증",
            ...
            },
            {
                ...
                detail: "6인 mesh WebRTC, 연결 시도별 순번으로 지연된 offer 및 answer 차단, DataChannel 수신 확인, RS256 참여권 검증과 짧은 TURN 접속 정보",
                evidence:
                    "BATON 서명자로 만든 정상 및 비정상 참여권을 로컬 HTTPS와 WSS에서 검증하고 Chromium 및 WebKit 브라우저 흐름 확인",
                input: "Core가 방 ID, 참가자 ID와 만료 시각을 넣어 발급한 짧은 RS256 참여권",
                output: "브라우저 사이의 offer, answer 및 ICE 중계, DataChannel 채팅 처리 결과와 짧은 TURN 접속 정보",
                recoveryBoundary:
                    "연결 시도 순번이 지난 answer 및 ICE는 버리고 정해진 횟수만 재연결하며 같은 참가자가 새 참여권으로 접속하면 이전 WebSocket 세션을 종료",
        ...
        oneLine:
            "Core는 조직과 접근 권한을 관리하고 짧은 링크, URL 점검, 알림 전송, 주간 보고서, 캘린더 구독과 WebRTC 스터디룸은 6개 서비스로 분리",
        status: {
            label: "현재 상태",
        ...
        visualCaption:
            "Core는 팀, 역할, 반복 업무, 의사결정, 인수인계와 접근 권한을 관리합니다. GO, WATCH, RELAY, BRIEF, CAL, ROUND는 각각 별도 저장소와 실행 환경에서 링크, URL 점검, 알림, 주간 보고서, 캘린더와 WebRTC 연결을 처리합니다.",
        problems: [
            {
                number: "01",
            ...
            },
            {
                ...
                decision:
                    "짧은 트랜잭션에서 SKIP LOCKED로 한 서버가 점검 작업만 가져간 뒤 DB 연결을 반환하고 외부 URL을 호출했습니다. DNS 조회 결과를 고정해 SSRF와 DNS 리바인딩을 차단했습니다. 작업 식별 토큰과 점검 요청 시점의 URL 버전을 비교해 이전 작업의 늦은 결과는 저장하지 않았습니다.",
                validation:
                ...
                print: {
                    ...
                    solution: "SSRF와 DNS 리바인딩 차단, 작업 식별 토큰과 요청 시점 URL 버전 비교",
            ...
            },
            {
                ...
                decision:
                    "외부 호출 전에 전송 시도 1건과 전송업체용 멱등 키를 DB에 저장하고 기존 시도 기록은 덮어쓰지 않았습니다. 결과를 확인할 수 없으면 결과 미확인 상태(OUTCOME_UNKNOWN)로 보존하고 운영자가 실제 전송 결과를 확인한 뒤에만 상태를 확정하도록 했습니다.",
                validation:
                ...
                print: {
                    ...
                    solution: "외부 호출 전 전송 시도 저장, 전송업체용 멱등 키와 결과 미확인 상태",
            ...
            },
            {
                ...
                decision:
                    "수락한 이벤트 전체를 다시 읽어 현재 확인 목록을 만들고, 같은 주간에 포함한 마지막 이벤트 위치와 선택 항목이 같으면 기존 보고서를 반환했습니다. 한 번 생성한 보고서는 수정하지 않고 당시 상태를 확인할 수 있도록 보관합니다.",
                validation:
                ...
                boundary:
                    "현재는 역할 미배정, 후임자 미지정, 인수인계 준비 미완료, 반복 지연 루틴과 미완료 인수인계를 처리합니다. 항목을 추가하면 BATON 이벤트 JSON, BRIEF의 현재 목록 생성 규칙과 주간 보고서 비교 규칙을 함께 변경해야 합니다.",
            },
            {
                number: "11",
                serviceIds: ["cal"],
            ...
            },
            {
                number: "13",
                serviceIds: ["round"],
                title: "이전 연결의 offer, answer와 ICE가 새 연결에 반영되지 않도록 차단",
                constraint:
                    "피어 연결을 다시 만들거나 ICE를 재시작한 뒤 이전 협상의 SDP와 ICE가 늦게 도착하면 새 연결 상태를 손상시킬 수 있습니다.",
                decision:
                    "피어 쌍마다 offer를 먼저 만드는 브라우저를 하나로 고정했습니다. 연결을 새로 시도할 때마다 순번을 올리고 answer와 ICE에도 같은 순번을 포함해 이전 연결 순번으로 늦게 도착한 메시지는 버립니다.",
                validation:
            ...
            },
            {
                ...
                constraint:
                    "offer, answer와 ICE를 중계할 때마다 ROUND가 Core를 호출하면 Core 응답 지연이나 장애 때문에 화상 연결도 멈춥니다. 반대로 ROUND가 계정과 멤버십을 저장하면 두 서비스의 권한 정보가 달라질 수 있습니다.",
                decision:
                ...
                boundary:
                    "멤버십 회수는 다음 참여권 갱신 또는 기존 참여권 만료까지 지연될 수 있습니다. 공인 DNS, 외부 coturn 미디어 중계, 실제 소셜 로그인과 6명 장시간 접속 테스트는 남아 있습니다.",
                print: {
    ...
    },
    {
        ...
        architecture: {
            ...
            tradeoff:
                "모듈과 타입 수는 늘지만 HTTP 요청, DB 저장과 외부 연동 코드가 업무 규칙을 거꾸로 참조하는지 빌드에서 확인할 수 있습니다. 현재 규모의 분리 비용을 고려해 도메인 모듈에는 일부 JPA 매핑을 유지했습니다.",
        },
        ...
        oneLine:
            "결제 결과를 받지 못한 재요청은 기존 처리 상태를 조회해 중복 승인 및 환불을 막고, 서버가 중단돼 남은 알림은 DB에서 다시 전송하며, 동시 예약과 주문은 좌석 및 옵션 재고를 잠근 뒤 처리",
        status: {
            label: "운영 상태",
        ...
        problems: [
            {
                number: "01",
                title: "웹 및 DB 코드가 핵심 업무 규칙을 거꾸로 참조하지 않도록 차단",
                constraint:
    ...
    },
    {
        ...
        proofs: [
            ...
            {
                item: "설정 파일, 토큰 및 인증 키 제외",
                method: "비공개 경로, 토큰 형태와 변경 파일 및 줄 위치 검증 테스트",
                rule: "리뷰가 추가로 열려고 한 비공개 설정 경로는 파일 본문을 읽기 전에 막고, 변경 파일에 토큰 및 인증 키 형태의 값이 있으면 검사 후 분석 입력과 HTML에서 제외",
    ...
    },
    {
        ...
        architecture: {
            ...
            description:
                "통신사실확인자료와 수신자료 조회에서 반복되는 분기와 오류 처리는 제네릭 메서드, enum, 공통 ErrorCode 및 Exception으로 묶었습니다. 기관별 조회, 변환과 전송 코드는 별도 클래스로 분리했습니다. 외부 API 호출 전후의 DB 반영만 별도 트랜잭션으로 실행해 응답 대기 중 DB 커넥션을 점유하지 않도록 했습니다.",
            tradeoff:
    ...
    },
    {
        ...
        problems: [
            ...
            {
                number: "02",
                title: "WebSquare 상태 변경 요청에 Spring Security CSRF 토큰 적용",
                constraint:
            ...
            },
            {
                ...
                decision:
                    "업로드 흐름을 `화면 → WAS 권한 및 파일 정보 검증 → Presigned URL 발급 → 파일 솔루션 직접 업로드` 순서로 변경했습니다. WAS는 업로드 가능 여부만 확인하고 파일 본문은 받거나 중계하지 않았습니다.",
                validation:
    ...
    },
    {
        ...
        proofs: [
            {
                ...
                method: "HLS 서버와 React 저장소의 구현 코드 및 6인 팀 시연 확인",
                rule: "mediasoup의 RTP 출력을 FFmpeg와 GStreamer에서 HLS로 변환하고, React 화면에서 WebRTC 실시간 영상과 HLS 지난 구간 영상을 각각 재생",
                result: "React 화면에서 현재 강의는 WebRTC로 시청하고 지나간 구간은 HLS로 다시 재생하는 흐름을 확인",
        ...
        oneLine:
            "mediasoup RTP 입력을 HLS로 변환하고 React에서 WebRTC 실시간 시청과 HLS 다시보기 제공",
        status: {
            label: "프로젝트 상태",
        ...
        problems: [
            {
                number: "01",
                title: "RTP 영상을 WebRTC 실시간 시청과 HLS 다시보기로 제공",
                constraint:
                    "현장 강의의 현재 영상은 낮은 지연으로 보여주면서, 수강자가 놓친 구간은 이전 시점으로 돌아가 다시 재생해야 했습니다. WebSocket은 연결 제어와 시그널링에 사용했기 때문에 실제 미디어를 전달하고 저장 가능한 형태로 변환하는 경로가 별도로 필요했습니다.",
                decision:
