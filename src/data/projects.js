...
const projects = [
    {
        ...
        architecture: {
            label: "서비스 구성과 담당 업무",
            title: "Core는 조직 운영, 6개 서비스는 링크, 점검, 이벤트, 보고서, 캘린더와 WebRTC를 담당합니다.",
            description:
                "Core는 조직과 인수인계를 관리합니다. 링크, URL 점검, 이벤트, 보고서, 캘린더와 WebRTC는 각 서비스가 처리하며 DB를 공유하지 않습니다.",
            tradeoff:
        ...
        services: [
            ...
            },
            {
                ...
                contribution:
                    "처리할 점검만 선점하고 외부 요청 중에는 DB 연결을 반환했습니다. 상태 변경 이벤트 재전송과 Prometheus 운영 지표도 구현했습니다.",
                stack: [
                    "Java 21",
                    "Spring Boot 4.1",
                    "Spring MVC",
            ...
            },
            {
                ...
                evidence: "Core-BRIEF 2.0.0-rc.1 로컬 HTTP, Caddy 예시 JSON 인증 및 수신 검증",
                ...
                status: "Core-BRIEF 2.0.0-rc.1 로컬 HTTP 연동을 확인했습니다. 2.0.0-rc.2, HTTPS와 원격 배포는 미검증입니다.",
                tradeoff:
        ...
        proofs: [
            ...
            },
            {
                ...
                method: "PostgreSQL 통합 테스트, Core-BRIEF 로컬 HTTP 연동과 Caddy HTTPS 예시 JSON 검증",
                rule: "같은 이벤트 재전송, 낮거나 건너뛴 개정 번호, 저장 이벤트 기반 운영 점검 목록 다시 생성과 동일 주간 보고서 동시 생성 요청을 실행",
                result: "중복 및 과거 이벤트를 무시하고 누락 개정을 기록했습니다. 목록은 같은 결과로 재생성됐고 보고서는 1건만 저장됐습니다.",
                scope: "Core-BRIEF 2.0.0-rc.1 로컬 HTTP 및 Caddy 예시 JSON 검증 · 2.0.0-rc.2, Core-Caddy HTTPS와 원격 배포 전 · 2026.08.27",
            },
            {
        ...
        oneLine:
            "Core가 조직과 인수인계를 관리하고, 6개 서비스가 링크, URL 점검, 이벤트, 보고서, 캘린더와 WebRTC를 담당합니다.",
        status: {
            label: "현재 상태",
            text: "Core와 6개 서비스의 주요 기능을 구현했습니다. CAL 및 ROUND 실제 데이터와 BRIEF 로컬 HTTP 연동은 확인했으며, BRIEF 최신 계약, HTTPS와 운영 자격 증명 전체 연동은 미검증입니다.",
        },
        visualCaption:
        ...
        problems: [
            ...
            },
            {
                ...
                decision:
                    "작업을 선점한 뒤 DB 연결을 반환하고 확인한 공인 IP로만 요청했습니다. 작업 ID나 URL 버전이 바뀐 결과는 저장하지 않았습니다.",
                validation:
            ...
            },
            {
                ...
                validation:
                    "2.0.0-rc.1로 Core와 BRIEF의 로컬 HTTP 연동과 Bearer 토큰 교체를 확인했습니다.",
                boundary: "2.0.0-rc.2 실제 JSON, Core-Caddy HTTPS와 원격 스테이징은 미검증입니다.",
            },
            {
                number: "10",
                serviceIds: ["brief"],
    ...
    },
    {
        ...
        category: "개인 프로젝트",
        role: "요구사항 정리, 웹 서비스 구현, 테스트와 설계 문서 작성",
        oneLine:
    ...
    },
    {
        ...
        architecture: {
            label: "선택한 두 커밋의 저장 코드만 검토",
            title: "현재 작업 파일을 제외하고 선택한 커밋의 변경만 검토합니다.",
            description:
                "일반 커밋은 첫 부모, 최초 커밋은 빈 상태, 병합 커밋은 선택한 부모와 비교합니다. 이후 작업 파일이 바뀌어도 확정한 두 커밋의 코드만 사용합니다.",
            tradeoff:
        ...
        proofs: [
            {
                ...
                result: "커밋 ID와 비교 기준을 고정하고 textconv 및 색상을 꺼도 이름 변경과 줄 수를 유지하며 비 UTF-8 경로는 거절",
            ...
            },
            {
                ...
                result: "비공개 경로와 자격 증명을 제외하고 수집 밖의 파일 및 줄을 가리킨 리뷰를 거절",
            ...
            },
            {
                ...
                result: "중단 후 빈 파일 없이 재개하고 다른 실행의 디렉터리와 기존 결과는 유지",
        ...
        oneLine:
            "선택한 로컬 커밋의 변경만 검토하고, 파일과 줄 근거를 확인한 결과를 새 HTML로 저장합니다.",
        status: {
            label: "공개 상태",
        ...
        problems: [
            {
                number: "01",
                title: "현재 작업 파일을 제외하고 선택한 커밋만 비교",
                constraint:
                    "스테이징한 파일, 수정 중인 파일과 추적하지 않는 파일을 함께 읽으면 특정 커밋에 없던 내용이 검토 결과에 섞일 수 있습니다.",
                decision:
                    "커밋 종류에 맞는 비교 기준을 확정하고 두 커밋에 저장된 코드만 읽습니다. 4.0.0에서는 textconv와 색상 출력을 끄고 UTF-8이 아닌 경로를 거절합니다.",
                validation:
            ...
            },
            {
                ...
                decision:
                    "별도 분석 작업의 각 설명을 실제 변경 파일과 줄에 연결하고 JSON Schema와 수집 범위로 검증합니다.",
                validation:
            ...
            },
            {
                ...
                decision:
                    "상태는 임시 파일에 저장한 뒤 교체하고 중단된 실행은 수집 지점부터 재개합니다. 저장 직전에는 실행과 검토 세대를 다시 확인합니다.",
                validation:
    ...
    },
    {
        ...
        proofs: [
            ...
            },
            {
                ...
                result: "신규 화면은 커서 다음 결과를 조회하고, 기존 화면은 번호 이동을 유지하며 키를 먼저 찾은 뒤 본문을 조회",
        ...
        architecture: {
            label: "공통 조회 로직 분리와 API 대기 중 DB 연결 반환",
            title: "조회, 변환과 전송을 분리하고 외부 API 대기 중 DB 트랜잭션을 종료했습니다.",
            description:
                "반복되는 조회와 오류 처리는 공통화하고 기관별 변환은 분리했습니다. 외부 API 호출 전후의 DB 반영만 별도 트랜잭션으로 실행했습니다.",
            tradeoff:
