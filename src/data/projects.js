...
const projects = [
    {
        ...
        evidenceAsOf:
            "2026.08.27 서비스별 기본 브랜치(main), 원격 개발 브랜치와 로컬 커밋을 각각 확인",
        ...
        architecture: {
            label: "서비스 구성과 담당 업무",
            title: "중앙 서비스(Core)는 조직 정보와 조직 작업 공간 접속용 공유 키를 관리하고, 링크 생성, URL 점검, 이벤트 전달, 주간 보고서, 캘린더 구독과 실시간 스터디룸은 6개 서비스가 나눠 처리합니다.",
            description:
                "Core는 팀, 시즌, 역할, 반복 업무, 의사결정과 인수인계를 저장하고 조직 작업 공간 접속용 공유 키를 확인합니다. 계정과 연결된 활동 중인 스터디 구성원 여부는 ROUND 참여권을 발급할 때 추가로 확인합니다. GO, WATCH, RELAY, BRIEF, CAL과 ROUND는 각각 별도 코드 저장소와 실행 환경으로 나눴습니다. DB가 필요한 서비스끼리 데이터를 공유하지 않으며, ROUND의 방과 연결 상태는 프로세스 메모리에 저장합니다.",
            tradeoff:
        ...
        spotlights: [
            {
                ...
                solution:
                    "클라이언트가 요청마다 보내는 고유 ID(UUID)의 SHA-256 해시로 기존 처리 기록을 찾습니다. 같은 UUID로 다시 요청하면 저장된 대상 시스템, 경로, 사용 목적, 활성 시각과 만료 시각을 직접 비교하고 하나라도 다르면 충돌로 거절합니다. 링크 코드는 UUID와 서버 비밀 키로 HMAC-SHA256을 계산해 생성합니다.",
                tradeoff:
            ...
            },
            {
                ...
                problem:
                    "외부 URL 응답을 기다리는 동안 DB 잠금을 유지하면 다른 점검이 대기하고, 늦게 끝난 작업이 최신 결과를 덮을 수 있었습니다.",
                solution:
                    "점검할 작업만 짧은 DB 트랜잭션에서 가져온 뒤 연결을 반환하고 외부 URL을 확인했습니다. URL의 호스트를 한 번 조회해 사설망 및 로컬 주소를 차단하고, 확인한 IP로만 연결해 DNS 변경을 이용한 내부망 접근을 막았습니다. 작업 식별자와 점검 시작 당시 URL 버전이 현재 값과 다르면 늦게 끝난 결과를 저장하지 않았습니다.",
                tradeoff:
            ...
            },
            {
                ...
                title: "전송 요청의 성공 여부를 모르면 다시 보내지 않음",
                problem:
                    "Webhook 수신 서버나 SQS API에 이벤트 전송을 요청한 뒤 RELAY가 응답을 받지 못하면, 성공 여부를 모른 채 다시 보내 같은 이벤트가 중복 전달될 수 있었습니다.",
                solution:
                    "받은 이벤트 ID를 수신 이력 테이블에 저장했습니다. 같은 이벤트가 다시 들어와도 동일한 전송 대상을 위한 작업을 새로 만들지 않아 재수신 때문에 전송이 한 번 더 시작되지 않도록 했습니다. 전송을 시작하기 전 발생한 재시도 가능한 일시 실패는 기존 작업으로 다시 처리하고, 전송 요청의 성공 여부를 확인하지 못하면 자동 재전송하지 않는 결과 미확인 상태(OUTCOME_UNKNOWN)로 남겼습니다.",
                tradeoff:
                    "중복 전달 방지를 우선해 자동 재전송을 멈추므로, 운영자가 Webhook 수신 기록이나 SQS 전송 기록 등 외부 증거를 확인하고 상태를 확정하는 절차가 필요합니다.",
            },
        ],
        documentGroups: [
            {
        ...
        services: [
            {
                ...
                summary:
                    "팀, 역할, 반복 업무, 의사결정과 인수인계를 저장하고 일반 조직 운영 요청의 조직 작업 공간 접속용 공유 키를 확인합니다. ROUND 참여권 발급 때에는 계정과 연결된 활동 중인 스터디 구성원인지 추가로 확인합니다.",
                detail: "팀, 시즌, 역할, 운영 기록, 조직 작업 공간 접속용 공유 키, 계정과 스터디 구성원 연결 및 ROUND 참여권 발급",
                evidence:
                ...
                inputRule:
                    "일반 조직 운영 요청은 조직 작업 공간 접속용 공유 키와 대상 데이터의 팀 및 시즌 소속을 확인합니다. 계정 세션과 계정에 연결된 활동 중인 스터디 구성원은 멤버십 관리와 ROUND 참여권 발급 요청에만 추가로 확인합니다.",
                ...
                status: "조직 운영 기능과 ROUND 참여권 발급, WATCH 상태 변경 이벤트 수신 및 CAL 일정 이벤트 생성을 구현했습니다. CAL 및 WATCH의 전달 대기, 실패, 처리 기한 만료와 마지막 성공 시각을 지표로 제공하고, 5분마다 지표 갱신 중단과 실패 항목을 검사해 로그를 남기는 운영 점검도 구현했습니다. 외부 알림과 실패 항목 자동 재처리는 연결하지 않았으며, 실제 OAuth 및 SMTP 계정과 운영용 자격 증명을 사용한 공개 HTTPS 환경 검증도 남아 있습니다.",
                documentation: [
                    { label: "PRD", count: "6" },
                    { label: "ADR", count: "19" },
            ...
            },
            {
                ...
                contribution:
                    "링크 생성과 조회, 활성 시작일 및 만료일, 폐기와 리다이렉트, 클라이언트가 링크 생성 요청마다 보내는 고유 ID(UUID)로 같은 요청의 중복 생성 차단과 HMAC 키 검증을 설계하고 구현했습니다.",
                stack: [
                    "Java 21",
                    "Spring Boot 4.1",
                    "Spring MVC",
            ...
            },
            {
                ...
                contribution:
                    "DB에서 다른 서버가 아직 가져가지 않은 점검만 선택하고 외부 요청 중에는 DB 연결을 반환하도록 구현했습니다. 상태 변경 이벤트는 URL 결과와 함께 저장하고 전송하지 못한 건은 다시 처리합니다. 점검 실행 수와 소요 시간, 최근 배치의 최대 일정 지연, 미전달 이벤트 수와 가장 오래된 대기 시간을 Prometheus 지표로 제공합니다.",
                stack: [
                    "Java 21",
                    "Spring Boot 4.1",
                    "Spring MVC",
                ...
                database: "PostgreSQL",
                visibility: "공개 저장소",
                status: "안전한 URL 점검, 상태 변경 이벤트 전송과 점검 지연 및 미전달 이벤트 대기 시간을 확인하는 운영 지표를 구현했습니다. 외부 대시보드와 알림을 연결한 공개 스테이징 이벤트 전송 테스트는 아직 하지 않았습니다.",
                tradeoff:
            ...
            },
            {
                ...
                contribution:
                    "이벤트 ID를 수신 이력 테이블에 저장하고 같은 이벤트가 다시 오면 기존 수신 결과를 반환했습니다. 동일한 전송 대상을 위한 작업을 새로 만들지 않아 재수신 때문에 전송이 한 번 더 시작되지 않도록 했습니다. 대상 호출 전에는 전송 시도와 대상용 중복 방지 키를 저장하고 RabbitMQ 수신, HTTP Webhook과 SQS FIFO 전송을 구현했습니다.",
                stack: [
                    "Java 21",
                    "Spring Boot 4.1",
                    "Spring MVC",
                ...
                detail: "같은 이벤트 재수신 시 전송 대상별 새 작업 생성 차단, DB에서 다른 서버가 처리하지 않은 작업 선택, 제한 횟수 재시도와 전송 성공 여부를 모르는 작업의 별도 보관",
                evidence:
                    "같은 이벤트 ID를 다시 받아도 같은 구독의 전달 작업이 추가되지 않고, 처리 서버 중단 뒤에는 기존 시도와 전송 식별자를 유지해 작업을 이어가는지 검증",
                input: "이벤트 ID, 이벤트 종류, 데이터 형식 버전, 대상 업무 식별자와 발생 시각",
                inputRule:
                    "이벤트 ID, 이벤트 종류, 데이터 형식 버전, 대상 업무 식별자와 발생 시각이 정해 둔 형식에 맞는지 확인합니다.",
                output: "Webhook 또는 SQS FIFO 전달 성공, 실패 또는 결과 미확인 상태",
                recoveryBoundary:
                    "같은 이벤트 ID를 다시 받으면 전송 대상별 새 작업을 추가하지 않습니다. 전송을 시작하기 전 발생한 재시도 가능한 일시 실패는 기존 작업으로 다시 처리하고, 전송 요청의 성공 여부를 알 수 없으면 자동 재시도를 중단합니다.",
                database: "PostgreSQL",
                visibility: "비공개 저장소 / 설계와 테스트 요약 공개",
                ...
                tradeoff:
                    "중복 전달 방지를 우선해 결과 미확인 건은 자동 재전송하지 않습니다. 운영자가 Webhook 수신 기록이나 SQS 전송 기록 등 외부 증거를 확인하고 상태를 확정해야 합니다.",
                documentation: [
                    { label: "PRD", count: "2" },
                    { label: "ADR", count: "15" },
                ],
            },
            {
                ...
                contribution:
                    "같은 이벤트와 과거 개정 번호는 운영 점검 목록에 다시 반영하지 않았습니다. 개정 번호가 건너뛴 최신 이벤트는 적용하되 누락된 개정 구간을 기록했습니다. 저장한 이벤트로 운영 점검 목록을 다시 만드는 기능, 주간 보고서 중복 생성 방지와 서비스 간 요청 인증 토큰(Bearer) 무중단 교체를 Kotlin과 Spring JDBC로 구현했습니다.",
                stack: [
                    "Kotlin 2.3",
                    "Java 21",
                    "Spring Boot 4.1",
                ...
                detail: "이벤트 중복 및 구버전 차단, 저장 이벤트로 운영 점검 목록 다시 생성, 생성 후 수정하지 않는 주간 보고서",
                evidence:
                    "실제 Core와 BRIEF 실행 JAR를 JSON 계약 패키지 2.0.0-rc.1 기준으로 로컬 HTTP에서 연결하고, 별도로 Caddy 내부 인증서 HTTPS 앞단에 계약 예시 JSON을 전송해 인증과 수신 검증",
                input: "담당자 및 후임자 공백, 위험 요소가 있는 역할의 책임 목록, 인수인계할 업무 항목과 역할 수행 자료 부족, 반복 마감 지연, 교대 시점이 가까운 미완료 인수인계 이벤트 및 각 개정 번호",
                inputRule:
                ...
                status: "JSON 계약 패키지 2.0.0-rc.1 기준으로 실제 Core와 BRIEF 실행 JAR를 로컬 HTTP에서 연결해 Core의 역할 및 인수인계 정보 변경이 BRIEF 운영 점검 목록에 반영되는지, 장애 재시도, 동일 이벤트 재전달과 현재 및 직전 Bearer 토큰을 함께 허용하는 교체 구간을 확인했습니다. 별도로 BRIEF의 Caddy 내부 인증서 HTTPS 앞단에는 계약 예시 JSON을 전송해 인증과 수신을 확인했습니다. 입력 규칙을 강화한 현재 2.0.0-rc.2는 Core의 실제 JSON 생성 코드와 다시 검증해야 하며, 실제 Core에서 Caddy로 보내는 HTTPS 전달도 아직 검증하지 않았습니다. 최신 구현은 원격 개발 브랜치에 있으며 공개 main 반영 전이고 원격 스테이징에도 배포하지 않았습니다.",
                tradeoff:
                    "각 항목이 선택된 이유를 이벤트 종류와 심각도로 확인할 수 있습니다. BRIEF는 Core가 판정한 다섯 가지 조직 연속성 신호를 담는 두 번째 이벤트 형식(v2)과 이전 형식(v1)을 함께 처리합니다. 이벤트 종류가 추가되면 Core가 보내는 JSON 형식, BRIEF의 운영 점검 목록 생성 규칙과 주간 보고서 비교 규칙을 두 버전에 맞춰 함께 변경해야 합니다.",
                repository: {
            ...
            },
            {
                ...
                contribution:
                    "Core가 확정한 일정 한 건의 전체 현재 값과 개정 번호를 JSON으로 보내면 저장하고 iCalendar 피드로 변환했습니다. 구독 토큰 회전과 폐기, 일정이 바뀌지 않았을 때 본문을 다시 보내지 않는 HTTP 캐시 응답도 구현했습니다.",
                stack: [
                    "Kotlin 2.3",
                    "Java 25",
                    "Spring Boot 4.1",
                ...
                detail: "외부 캘린더용 .ics 피드, 구독 주소의 토큰 교체 및 폐기, 일정이 바뀌지 않았을 때 304를 반환하는 캐시 처리",
                evidence:
                    "Core의 실제 일정 JSON 생성 코드로 만든 데이터를 CAL 컨테이너에 전송해 중복 및 과거 일정 차단과 iCalendar 결과를 검증",
                ...
                status: "시즌 일정의 수신, 변경, 취소와 읽기 전용 iCalendar 구독 기능을 구현했습니다. 안정 계약 1.0.0 기준으로 Core의 실제 일정 JSON 생성 코드로 만든 데이터를 CAL 컨테이너에 보내 정상 반영되는 것도 확인했습니다. 개발 브랜치의 1.1.0-rc.1에는 JSON 요청 크기와 구조 제한 및 운영 환경에서 로컬 DB 기본값을 사용하지 못하게 하는 검사를 추가했지만, 아직 공개 릴리스로 게시하거나 Core에 적용하지 않았습니다. 운영 자격 증명을 사용한 실제 이벤트 전송과 공개 캘린더 앱 구독도 아직 하지 않았습니다.",
                tradeoff:
            ...
            },
            {
                ...
                detail: "참가자마다 나머지 최대 5명과 직접 연결하는 mesh WebRTC, 이전 연결의 늦은 메시지 차단, DataChannel 채팅이 상대 브라우저 애플리케이션에 도착했는지 확인하는 응답, Core가 RSA 개인 키로 서명하고 ROUND가 공개 키로 검증하는 RS256 참여권과 짧은 TURN 접속 정보",
                evidence:
                    "Chromium에서는 가짜 카메라, 마이크와 화면으로 원격 미디어, 화면 공유, 채팅 수신 응답과 퇴장을 확인했고, WebKit에서는 직접 초대와 장치 권한 동의까지만 확인",
                ...
                visibility: "비공개 저장소 / 설계와 테스트 요약 공개",
                status: "원격 개발 브랜치에서 Core의 실제 참여권 서명 코드로 만든 참여권을 ROUND가 로컬 HTTPS와 암호화된 WebSocket(WSS)에서 검증하고 방에 입장시키는 과정을 확인했습니다. 공인 DNS와 자동 발급 TLS 인증서, 외부 TURN 미디어 중계 서버(coturn)와 6명 장시간 접속 테스트는 아직 하지 않았습니다.",
                tradeoff:
        ...
        proofs: [
            {
                ...
                result: "준비 → 전달 → 수락 순서만 허용하고 역할별 진행 중인 인수인계를 1건으로 유지",
            ...
            },
            {
                ...
                method: "PostgreSQL 통합 테스트, Core 실행 JAR에서 BRIEF 실행 JAR로 로컬 HTTP 전송 및 Caddy 내부 인증서 HTTPS 앞단에 계약 예시 JSON 전송",
                rule: "같은 이벤트 재전송, 낮거나 건너뛴 개정 번호, 저장 이벤트 기반 운영 점검 목록 다시 생성과 동일 주간 보고서 동시 생성 요청을 실행",
                result: "중복 및 과거 이벤트는 반영하지 않고, 건너뛴 최신 개정은 누락 구간을 기록한 뒤 반영했습니다. 저장한 이벤트에서 같은 운영 점검 목록을 다시 만들고 동일 상태의 주간 보고서는 1건만 저장했습니다.",
                scope: "실제 Core와 BRIEF 실행 JAR 연결은 JSON 계약 패키지 2.0.0-rc.1 및 로컬 HTTP 기준 · Caddy HTTPS는 계약 예시 JSON만 검증 · 현재 2.0.0-rc.2의 실제 Core JSON, Core-Caddy HTTPS, 공개 main 및 원격 스테이징 반영 전 · 2026.08.27",
            },
            {
        ...
        category: "개인 프로젝트",
        role: "중앙 서비스와 6개 서비스의 API, 서비스별 저장 방식, 이벤트 형식, 중복 및 실패 재처리 테스트와 배포 절차 설계 및 구현",
        oneLine:
            "Core는 조직 데이터, 조직 작업 공간 접속용 공유 키와 ROUND 방 참여 가능 여부를 관리합니다. GO, WATCH, RELAY, BRIEF, CAL, ROUND는 각각 짧은 링크, URL 점검, 서비스 간 이벤트 전달, 주간 보고서, 캘린더 구독과 WebRTC 스터디룸을 처리합니다.",
        status: {
            label: "현재 상태",
            text: "Core와 6개 서비스의 주요 기능을 구현했습니다. CAL은 Core의 실제 일정 JSON 생성 코드로 만든 데이터를 컨테이너에서 처리했고, ROUND는 Core의 실제 참여권 서명 코드로 만든 참여권을 HTTPS 및 암호화된 WebSocket(WSS)에서 검증했습니다. BRIEF는 JSON 형식 2.0.0-rc.1 기준으로 실제 Core 실행 JAR와 로컬 HTTP에서 연동했고, Caddy HTTPS는 계약 예시 JSON으로 별도 확인했습니다. 현재 2.0.0-rc.2의 실제 Core JSON 재검증, 실제 Core-Caddy HTTPS 전달과 공개 main 반영이 남아 있습니다. 실제 운영 자격 증명과 외부 서비스를 사용하는 공개 환경의 전체 연동 검증도 아직 완료하지 않았습니다.",
        },
        visualCaption:
            "Core는 팀, 역할, 반복 업무, 의사결정, 인수인계와 조직 작업 공간 접속용 공유 키를 관리합니다. GO, WATCH, RELAY, BRIEF, CAL과 ROUND는 각각 별도 코드 저장소와 실행 환경으로 나눴습니다. DB가 필요한 서비스끼리 데이터를 공유하지 않고, ROUND의 방과 연결 상태는 프로세스 메모리에 저장합니다.",
        problems: [
            {
                number: "01",
                serviceIds: ["core", "go", "watch", "relay", "brief", "cal", "round"],
                shared: true,
                title: "Core와 6개 서비스의 담당 업무, 실행 환경과 데이터 저장 방식 분리",
                constraint:
                    "링크 생성, URL 점검, 서비스 간 이벤트 전달, 주간 보고서, 캘린더 구독과 실시간 통신은 받는 데이터, 보안 검사와 실패 후 처리 방식이 서로 다릅니다.",
                decision:
                    "Core에는 조직, 역할, 인수인계와 조직 작업 공간 접속용 공유 키를 두고 GO, WATCH, RELAY, BRIEF, CAL과 ROUND는 각각 별도 코드 저장소와 실행 환경으로 분리했습니다. DB가 필요한 서비스끼리 데이터를 공유하지 않으며 ROUND의 방과 연결 상태는 프로세스 메모리에 저장합니다. 다른 서비스에 알려야 하는 상태 변경은 업무 데이터와 함께 전송 대기 이벤트로 DB에 저장하고, 업무 저장이 끝난 뒤 전송합니다.",
                validation:
                    "GO는 같은 링크 요청 8건의 단일 저장, WATCH는 사설망 URL과 늦은 점검 결과 차단, RELAY는 RabbitMQ 재전달 중복 제거, BRIEF는 운영 점검 목록 다시 생성과 주간 보고서 중복 생성 방지, CAL은 중복 일정과 iCalendar 생성, ROUND는 잘못된 참여권과 지연 WebRTC 메시지 차단을 각각 확인했습니다.",
                boundary:
                    "서비스별로 기본 브랜치(main), 개발 브랜치와 로컬 구현 상태가 다릅니다. 위에 적은 기능별 테스트는 완료했지만 모든 서비스를 공개 환경에 배포해 실제 자격 증명과 외부 서비스를 사용하는 전체 흐름은 아직 확인하지 않았습니다.",
            },
            {
                number: "02",
                serviceIds: ["core"],
                ...
                decision:
                    "준비 단계에서 현재 담당자, 다음 담당자와 다음 담당 기간을 고정했습니다. 전달할 때 인수인계 업무 항목 수, 미완료 항목 수와 역할 자료 수를 저장하고, 업무 항목이 없거나 미완료 항목이 있거나 자료가 없으면 경고 확인을 요구했습니다. 전달 뒤에는 수락 또는 취소 전까지 역할, 인수인계 업무와 자료를 수정하지 못하게 했고, 수락할 때 담당자와 담당 기간을 한 트랜잭션에서 변경했습니다.",
                validation:
                ...
                print: {
                    ...
                    solution: "준비 → 전달 → 수락 단계와 역할별 진행 중인 인수인계 1건 제약",
            ...
            },
            {
                ...
                decision:
                    "클라이언트가 링크 생성 요청마다 보내는 고유 ID(UUID)의 SHA-256 해시로 처리 기록을 찾습니다. 재요청이면 저장된 대상 시스템, 경로, 사용 목적, 활성 시각과 만료 시각을 직접 비교하고 하나라도 다르면 충돌로 거절합니다. 링크 코드는 같은 UUID와 서버 비밀 키로 HMAC-SHA256을 계산해 생성합니다.",
                validation:
            ...
            },
            {
                ...
                constraint:
                    "잘못된 HMAC 키로 서버가 시작되면 같은 목적지에 다른 링크 코드가 생겨 기존 링크 주소를 더 이상 유지할 수 없었습니다.",
                decision:
            ...
            },
            {
                ...
                print: {
                    label: "WATCH / URL 보안",
                    problem:
                        "느린 외부 URL 응답으로 DB 연결 대기가 늘고, 늦게 끝난 점검이 최신 상태를 덮을 수 있음",
            ...
            },
            {
                number: "07",
                serviceIds: ["relay"],
                title: "외부 전송 요청의 성공 여부를 모를 때 이벤트 중복 전달 방지",
                constraint:
                    "Webhook 수신 서버나 SQS API에 이벤트 전송을 요청한 뒤 RELAY가 응답을 잃으면, 성공 여부를 모른 채 다시 보내 같은 이벤트가 중복 전달될 수 있었습니다.",
                decision:
                    "대상 호출 전에 전송 시도 1건과 대상용 중복 방지 키를 DB에 저장하고 기존 시도 기록은 덮어쓰지 않았습니다. 전송 요청의 성공 여부를 확인할 수 없으면 결과 미확인 상태(OUTCOME_UNKNOWN)로 보존하고, 운영자가 Webhook 수신 기록이나 SQS 전송 기록 등 외부 증거를 확인한 뒤에만 상태를 확정하도록 했습니다.",
                validation:
            ...
            },
            {
                ...
                decision:
                    "받은 이벤트 ID를 수신 이력 테이블에 저장해 같은 이벤트가 다시 오면 등록된 전송 대상별 새 작업을 만들지 않았습니다. DB 저장 뒤 RabbitMQ에 처리 완료 응답(ACK)을 보내고, 반복해서 처리할 수 없는 메시지는 별도 실패 큐(DLQ)로 분리했습니다.",
                validation:
            ...
            },
            {
                ...
                decision:
                    "이벤트 ID, 내용 해시와 개정 번호를 함께 저장했습니다. 같은 이벤트와 과거 개정 번호는 운영 점검 목록에 다시 반영하지 않고, 개정 번호가 건너뛴 최신 이벤트는 반영하되 누락된 개정 구간을 함께 기록했습니다. 수락한 이벤트 기록과 운영 점검 목록은 같은 PostgreSQL 트랜잭션에서 저장합니다.",
                validation:
                    "PostgreSQL 통합 테스트와 JSON 계약 패키지 2.0.0-rc.1을 사용한 실제 Core-BRIEF 실행 JAR의 로컬 HTTP 전송으로 같은 이벤트와 과거 개정 번호를 반영하지 않고, 건너뛴 최신 개정은 누락 구간을 기록한 뒤 반영하는지 확인했습니다. 현재 Bearer와 직전 Bearer를 잠시 함께 허용해 전송 중단 없이 토큰을 교체하는 과정도 확인했습니다. Caddy 내부 인증서 HTTPS 앞단의 인증과 수신은 계약 예시 JSON으로 별도 확인했습니다.",
                boundary:
                    "실제 Core와 BRIEF 실행 JAR를 로컬 HTTP로 연결해 검증했고, Caddy 내부 인증서 HTTPS에는 계약 예시 JSON만 전송했습니다. 입력 규칙을 강화한 현재 2.0.0-rc.2는 Core의 실제 JSON 생성 코드와 다시 검증해야 합니다. 실제 Core에서 Caddy로 보내는 HTTPS 전달, 공개 main 반영과 공인 DNS를 사용한 원격 스테이징 전달도 아직 검증하지 않았습니다.",
                print: {
                    label: "BRIEF / 이벤트 중복 차단",
                    problem: "중복 또는 이전 버전 이벤트가 운영 점검 항목을 바꿀 수 있음",
                    solution: "이벤트 ID, 내용 해시와 개정 번호를 같은 트랜잭션에서 확인",
                    tradeoff:
                        "2.0.0-rc.1 실제 Core 연동은 로컬 HTTP, Caddy HTTPS는 예시 JSON만 확인",
                },
            },
            {
                number: "10",
                ...
                decision:
                    "수락한 이벤트 전체를 다시 읽어 운영 점검 목록을 만들었습니다. 보고서를 만들 때 BRIEF가 이벤트를 받은 순서(수신 순번)와 포함 항목을 함께 저장하고, 같은 주간과 같은 수신 순번 및 항목으로 다시 요청하면 기존 보고서를 반환했습니다. 한 번 생성한 보고서는 수정하지 않고 당시 상태를 확인할 수 있도록 보관합니다.",
                validation:
                    "같은 이벤트 기록으로 운영 점검 목록을 다시 만들기 전후의 결과와 같은 주간 보고서를 동시에 생성하는 요청을 확인했습니다. 동시 요청에서도 보고서는 1건만 저장되고 최초 생성은 HTTP 201, 같은 요청의 재실행은 HTTP 200으로 구분됩니다.",
                boundary:
                    "Core 개발 브랜치가 생성하는 두 번째 이벤트 형식(v2)에는 담당자가 없는 역할, 담당 종료가 임박했지만 후임자가 없는 역할, 위험 요소가 기록됐고 책임 목록이 없거나 인수인계에 포함할 업무 항목이 없거나 끝나지 않은 항목이 있거나 역할 수행 자료가 없는 역할, 반복해서 마감이 지난 업무와 담당 교대 시점이 가까운데 인수인계를 시작하지 않았거나 전달 및 수락 조건이 남은 경우가 포함됩니다. 이전 이벤트 형식(v1)도 계속 처리하므로 항목을 추가할 때 두 버전의 이벤트 JSON, 운영 점검 목록 생성 규칙과 주간 보고서 비교 규칙을 함께 변경해야 합니다. 입력 규칙을 강화한 JSON 계약 패키지 2.0.0-rc.2는 Core의 실제 JSON 생성 코드와 다시 검증해야 합니다.",
            },
            {
                number: "11",
                serviceIds: ["cal"],
            ...
            },
            {
                number: "12",
                serviceIds: ["cal"],
                title: "시간대와 취소 상태를 정확히 표시하고 일정이 바뀌지 않으면 본문 생략",
                constraint:
                    "캘린더 앱마다 시간대와 취소 일정 및 캐시 처리 방식이 달라 일정이 중복되거나 변경 내용이 반영되지 않을 수 있습니다.",
                decision:
                    "일정 식별값(UID)은 고정하고 원본 개정 번호를 캘린더 변경 순번(SEQUENCE)으로 사용해 변경과 취소를 구분했습니다. 일정이 바뀌지 않았으면 같은 캐시 확인값(ETag)을 반환하고 본문 대신 304 Not Modified로 응답했습니다.",
                validation:
            ...
            },
            {
                ...
                print: {
                    label: "ROUND / 재연결",
                    problem: "이전 연결의 설정 정보가 늦게 도착해 새 연결을 손상시킬 수 있음",
                    solution:
                        "연결을 먼저 시작할 브라우저를 고정하고 이전 연결 순번의 메시지를 차단",
                    tradeoff: "브라우저 코드와 연결 메시지 규격을 함께 배포해야 함",
                },
            },
            {
                number: "14",
                ...
                constraint:
                    "브라우저의 연결 설명과 네트워크 경로 후보를 전달할 때마다 ROUND가 Core를 호출하면 Core 응답 지연이나 장애 때문에 화상 연결도 멈춥니다. 반대로 ROUND가 계정과 스터디 구성원 정보를 저장하면 두 서비스의 권한 정보가 달라질 수 있습니다.",
                decision:
                    "Core가 계정에 연결된 활동 중인 스터디 구성원인지 확인한 뒤 방 ID와 만료 시각을 넣어 RS256 참여권을 발급합니다. ROUND는 Core의 공개 키로 쿠키의 서명, 방 ID와 만료 시각을 직접 확인하고, 통과한 연결의 WebSocket 중계와 TURN 접속 정보 발급만 담당합니다.",
                validation:
                ...
                boundary:
                    "스터디 구성원 자격을 회수해도 다음 참여권 갱신 또는 기존 참여권 만료까지 반영이 늦어질 수 있습니다. 공인 DNS와 인증서, 외부 TURN 미디어 중계 서버(coturn), 실기기 Safari와 6명 장시간 접속 테스트는 남아 있습니다.",
                print: {
                    label: "ROUND / 방 입장 권한",
                    problem:
                        "WebRTC 메시지마다 Core를 호출하면 Core 장애로 화상 연결도 멈출 수 있음",
                    solution:
                        "Core가 개인 키로 서명한 참여권을 발급하고 ROUND가 공개 키로 직접 확인",
    ...
    },
    {
        ...
        evidenceAsOf: "2026.08.27 로컬 커밋 b50a9ef0 기준 · 공개 main 및 원격 개발 브랜치 미반영",
        ...
        spotlights: [
            {
                ...
                problem:
                    "결제사 실패 상태를 저장한 뒤 예외를 던지면 같은 트랜잭션이 롤백되어 이력까지 사라졌습니다. 응답 유실 뒤 재요청은 중복 승인이나 환불로 이어질 수 있었습니다.",
                solution:
                ...
                tradeoff:
                    "결제사 호출을 트랜잭션 밖으로 분리해 DB 점유는 줄지만, API 응답 시점에 환불 상태가 요청됨(REQUESTED)으로 남을 수 있습니다.",
            },
            {
                label: "서버 중단으로 남은 알림 재전송",
        ...
        proofs: [
            ...
            },
            {
                ...
                result: "SKU별 요구 수량을 합산하고 SKU ID 순서로 DB 재고 행을 잠가 차감하며, 주문에 저장한 결제 당시 옵션과 가격으로 과거 주문 조건을 재현",
                scope: "2026.08.27 로컬 커밋 b50a9ef0 기준 · 원격 브랜치와 공개 main 반영 전",
            },
            {
                ...
                result: "외부 등록 실패는 DB 상태를 기준으로 배치가 다시 요청하고, HMAC-SHA256 서명과 전송 시각을 확인한 웹훅만 배송 상태와 이력에 반영하며 택배사 배송 완료만으로 주문을 완료하지 않음",
        ...
        status: {
            label: "운영 상태",
            text: "AWS 운영 환경에 배포했으나 상시 리소스 비용으로 운영을 종료했습니다. 현재 공개 URL은 없습니다. 최신 로컬 브랜치에는 공개 화면 서버 렌더링, 옵션 조합별 재고, 예약 캘린더와 클래스별 정원, 배송조회, 취소 전 환불 안내와 미수령 주문의 관리자 예외 환불, 관리자 30분 예약 시간표, 홈 공지 및 이벤트, 이벤트 쿠폰, 카카오 로그인과 계정 연결까지 구현했습니다. 기준 커밋은 2026.08.27 b50a9ef0이며 아직 원격 개발 브랜치와 공개 main에는 반영하지 않았습니다.",
        },
        visualCaption:
        ...
        problems: [
            {
                number: "01",
                title: "주문 및 예약 규칙을 웹 요청과 DB 저장 코드에서 분리",
                constraint:
            ...
            },
            {
                ...
                validation:
                    "예외 뒤에도 실패 이력이 남는지, 같은 요청 ID를 다시 보내면 기존 결과를 반환하는지 확인했습니다. 이전 재시도의 늦은 응답이 최신 처리 상태를 덮지 않는지, 승인 응답이 늦게 도착해도 승인 완료 상태가 반영되는지, 결과를 확인하지 못한 환불을 다시 조회해 복구하는지도 통합 테스트로 확인했습니다.",
                boundary:
            ...
            },
            {
                ...
                validation:
                    "같은 알림 작업이 DB에 두 번 저장되지 않는지, 처리 서버가 중단된 뒤 다른 서버가 이어받는지, 실패 후 다시 처리되는지, 발송 직전에 대상을 다시 확인하는지 통합 테스트로 확인했습니다.",
                boundary:
            ...
            },
            {
                ...
                decision:
                    "클래스별 정원을 예약 시간에 저장하고, 예약은 클래스 행과 예약 시간 행을 잠근 뒤 현재 예약 인원과 요청 인원의 합을 확인했습니다. 일반 상품 재고는 상품 ID, 옵션별 재고는 SKU ID 오름차순으로 잠그고 수량 확인과 차감을 한 트랜잭션에서 처리했습니다.",
                validation:
                    "정원 3명인 예약 시간의 네 번째 요청과 남은 자리 1개에 대한 동시 예약을 거절하는지 확인했습니다. 마지막 재고에도 동시 주문을 보내 한 건만 성공하고 나머지는 같은 업무 오류를 반환하는지 검증했습니다.",
                boundary:
                ...
                print: {
                    ...
                    solution:
                        "예약은 클래스 행 다음 예약 시간 행을 잠그고, 재고는 일반 상품 ID 또는 옵션 SKU ID 순서로 잠근 뒤 수량 확인 및 차감",
            ...
            },
            {
                ...
                decision:
                    "복원이 필요한 값은 AES-GCM으로 암호화하고, 정확 검색에는 원문을 복원하지 않는 검색용 HMAC 해시(블라인드 인덱스)를 사용했습니다. 기존 데이터 전환과 키 회전도 별도 단계로 설계했습니다.",
                validation:
                ...
                print: {
                    ...
                    solution: "AES-GCM 암호화와 검색용 HMAC 해시",
            ...
            },
            {
                ...
                print: {
                    ...
                    tradeoff: "결제사 환불 완료 전에 예약 취소와 크레딧 변경이 먼저 끝날 수 있음",
                },
            },
            {
                number: "08",
                ...
                constraint:
                    "기존 브라우저 렌더링 방식(SPA)은 JavaScript 실행 전 공개 상품과 클래스 본문이 없고, 존재하지 않는 경로도 HTTP 200과 공통 메타데이터를 반환했습니다. 회원, 결제와 관리자 화면까지 서버 렌더링(SSR)하면 개인별 세션과 캐시가 섞일 수 있었습니다.",
                decision:
                    "공개 상품과 클래스 화면은 서버가 본문, 대표 주소(canonical), 공유 미리보기 정보(Open Graph)와 검색엔진용 구조화 데이터(JSON-LD)를 포함한 HTML로 응답하도록 구현했습니다. 존재하지 않는 상세는 HTTP 404를 반환했습니다. 회원, 결제와 관리자 화면은 브라우저에서만 렌더링하고 검색 제외 지시(noindex)를 유지했습니다.",
                validation:
            ...
            },
            {
                ...
                boundary:
                    "같은 클래스의 예약 시간 조회와 예약 확정은 클래스 행을 차례로 잠그기 때문에 조회가 몰릴 때 대기 시간을 관찰해야 합니다. 임시공휴일과 선거일은 날짜 차단으로 보완해야 하며, 예약 캘린더 구현은 2026.08.27 로컬 커밋 b50a9ef0 기준으로 원격 개발 브랜치와 공개 main 반영 전입니다.",
                print: {
            ...
            {
                number: "11",
                title: "외부 배송조회 등록 실패를 다시 처리하고 배송 완료와 주문 완료를 분리",
                constraint:
                    "관리자가 운송장을 저장한 뒤 외부 배송조회 등록이 실패하면 고객은 배송 경로를 볼 수 없습니다. 반대로 택배사의 배송 완료 웹훅만으로 주문을 완료하면 적립금 지급과 후기 요청 같은 업무 처리가 실제 인도 확인보다 먼저 실행될 수 있었습니다.",
                decision:
                    "택배사 코드, 표시명과 운송장 번호를 주문 이행 정보에 저장하고 외부 배송조회 등록 상태는 별도로 관리했습니다. 429, 5xx 및 통신 실패는 배치가 DB 상태를 읽어 다시 요청합니다. 웹훅은 HMAC-SHA256 서명과 5분 이내 전송 시각을 확인한 뒤 배송 상태와 이동 이력만 갱신하며, 주문 완료는 관리자가 별도로 확정합니다.",
                validation:
                    "제작 완료부터 운송장 등록, 외부 배송조회 등록, 배송 완료 웹훅과 관리자 주문 완료까지 통합 테스트로 확인했습니다. 올바른 서명과 허용 시각의 웹훅만 처리하고, 택배사 배송 완료 뒤에도 주문 상태가 배송 중으로 유지되는지 검증했습니다.",
                boundary:
                    "현재는 Delivery API 한 곳과 지원 택배사 목록을 기준으로 구현했습니다. 실제 운영 자격 증명, 외부 서비스 장애 시간과 웹훅 재전달을 포함한 운영 환경 검증은 아직 하지 않았습니다.",
                print: {
                    label: "SHIPPING / TRACKING",
                    problem: "외부 배송조회 등록 실패와 택배사 완료의 조기 주문 완료 위험",
                    solution: "등록 상태 배치 재처리, 서명된 웹훅과 관리자 주문 완료 분리",
                    tradeoff: "외부 Delivery API 운영 환경 검증 전",
                },
            },
        ],
        stack: [
    ...
    },
    {
        ...
        evidenceAsOf:
            "2026.08.27 기준 · 공개 main 3.1.1과 원격 개발 브랜치 4.0.0 커밋 d6203de의 구현 상태를 구분해 표기",
        evidenceTitle: "구현 및 자동화 테스트",
        systemTitle: "커밋 검토 처리 흐름",
        systemNavLabel: "처리 흐름",
        architecture: {
            label: "입력한 커밋과 커밋 종류별 규칙으로 확정한 비교 기준의 코드만 사용",
            title: "현재 수정 중인 파일을 제외하고, 사용자가 입력한 커밋과 확정한 비교 기준 사이의 변경만 검토합니다.",
            description:
        ...
        proofs: [
            {
                ...
                result: "전체 커밋 ID와 비교 기준을 고정하고, 로컬 Git의 textconv 및 색상 설정을 끈 상태에서도 파일 이름 변경과 변경 줄 수를 유지하며 UTF-8이 아닌 경로는 분석 전에 거절",
            ...
            },
            {
                ...
                rule: "입력 커밋과 비교 기준을 확정하고 리뷰 설명의 파일, 줄과 JSON 형식을 확인한 뒤, 저장 직전 같은 실행과 검토 세대인지 다시 확인한 경우에만 새 HTML 파일 생성",
                result: "상태 기록 중 중단돼도 빈 파일을 남기지 않고 같은 실행을 재개하며, 다른 실행이 바꾼 디렉터리나 기존 결과 파일은 덮어쓰지 않음",
        ...
        oneLine:
            "입력한 로컬 커밋을 일반, 최초 및 병합 커밋별 규칙으로 확정한 비교 기준과 대조해 변경 코드와 리뷰를 새 HTML 파일로 저장하되, 현재 수정 중인 파일과 이전 대화는 분석에서 제외",
        status: {
            label: "공개 상태",
            text: "SeungIl 님이 개발한 Hope 3.0.3의 Git 이력, MIT 라이선스와 기존 기능을 유지한 비공식 포크입니다. 제가 추가한 Commit Diff와 관련 테스트 및 문서는 README와 NOTICE에 구분했습니다. 공개 main은 3.1.1입니다. 원격 개발 브랜치 4.0.0에서는 비공개 경로와 토큰 차단에 더해 Git의 textconv 및 색상 설정을 무시하고 UTF-8이 아닌 파일 경로를 거절하며, 중단된 리뷰 수집을 같은 실행에서 재개하도록 보완했습니다. 4.0.0은 아직 main과 태그에 반영하지 않았습니다.",
        },
        visualCaption:
            "입력 커밋과 커밋 종류별 비교 기준을 먼저 확정하고 현재 수정 파일을 제외한 변경 코드를 수집한 뒤, 리뷰 설명이 실제 파일과 줄을 가리키는지 확인한 결과만 새 HTML로 저장합니다.",
        problems: [
            {
                number: "01",
                ...
                decision:
                    "입력한 16진수 커밋 ID를 전체 ID로 확정했습니다. 일반 커밋은 첫 번째 부모, 최초 커밋은 빈 상태, 병합 커밋은 사용자가 고른 부모를 비교 기준으로 삼았습니다. 파일 본문과 변경 내역은 현재 수정 중인 파일이 아니라 확정한 커밋과 비교 기준에 저장된 코드에서 읽습니다. 4.0 개발 브랜치에서는 사용자 Git 설정의 textconv 및 색상 출력을 끄고, 파일 경로를 바이트 단위로 확인해 UTF-8이 아니면 분석 전에 거절합니다.",
                validation:
                    "짧은 커밋 ID, 최초 커밋, 병합 커밋의 부모 선택과 파일 이름 변경을 테스트용 Git 저장소로 확인했습니다. textconv가 설정돼도 실행되지 않는지, 색상 코드가 섞이지 않는지와 UTF-8이 아닌 파일 경로를 거절하는지도 검증했습니다.",
                boundary:
            ...
            },
            {
                ...
                validation:
                    "수집한 변경 코드에 없는 파일이나 줄을 참조한 설명, 존재하지 않는 줄 범위, 정해진 결과 형식에 없는 필드와 과도하게 긴 설명을 거절하는 테스트로 확인했습니다.",
                boundary:
                    "별도 분석 작업을 사용할 수 없으면 검토를 중단합니다. 코드 줄 연결 여부는 확인할 수 있지만 리뷰 판단이 반드시 옳다는 뜻은 아니므로 사용자가 최종 확인해야 합니다.",
            },
            {
                number: "04",
                title: "검증이 끝난 결과만 저장하고 중단된 검토는 같은 실행에서 재개",
                constraint:
                    "검토 상태를 기록하다 중단되면 빈 파일이 남거나 처음부터 다시 수집할 수 있습니다. 저장 직전에 다른 실행이 같은 작업 디렉터리를 교체하면 기존 결과를 덮어쓸 위험도 있습니다.",
                decision:
                    "상태 파일은 임시 파일에 먼저 기록한 뒤 한 번에 교체해 중단돼도 빈 파일을 남기지 않습니다. 같은 실행의 수집 상태를 보존해 중단 지점부터 다시 진행하고, HTML 저장 직전에는 작업 잠금을 얻은 뒤 실행 디렉터리와 최신 검토 세대를 다시 확인합니다. 다른 실행이 바꾼 디렉터리와 기존 출력 파일은 덮어쓰지 않습니다.",
                validation:
                    "상태 기록 단계별 중단과 재개, 검증 전후 대상 변경, 잠금 소유권 상실, 기존 출력 경로, 심볼릭 링크와 동시 저장을 재현해 다른 실행의 작업 디렉터리와 기존 HTML을 바꾸지 않는지 확인했습니다.",
                boundary:
    ...
    },
    {
        ...
        proofs: [
            ...
            },
            {
                ...
                result: "신규 화면은 마지막 조회 키 다음 데이터부터 이어지고, 기존 화면은 페이지 번호 이동과 목록 결과를 유지하면서 본문 조회를 뒤로 미룬 SQL이 적용되는 것을 확인",
            ...
            },
            {
                ...
                result: "재시도 간격을 점차 늘리고 무작위 지연을 더해 요청 상태를 다시 조회한 뒤 완료 결과를 반영했습니다(Spring Retry).",
                scope: "보안상 운영 수치와 테스트 코드는 비공개",
            },
        ],
        category: "BEINTECH / LG CNS 컨소시엄 공공 SI",
        role: "KICS의 자료 제공 요청을 통신사와 집행포털 규격으로 변환해 전송하고, 통신사와 집행포털 연계를 통해 수신한 제출 자료를 KICS에 반영하는 서버 기능과 Spring Batch 구현",
        oneLine:
        ...
        visualCaption:
            "전체 시스템은 KICS, 집행포털, 금융기관 및 통신사의 독립망 사이에서 요청과 제출 자료를 중계합니다. 이 가운데 KICS 요청을 통신사와 집행포털 규격으로 변환해 보내고 제출 자료를 다시 KICS에 반영하는 서버 기능과 배치를 담당했습니다.",
        architecture: {
            label: "반복 조회 코드는 공유하고 외부 API 대기 중 DB 연결 반환",
        ...
        problems: [
            {
                ...
                decision:
                    "신규 화면은 마지막으로 본 전송 ID 다음부터 조회했습니다. 페이지 번호 이동이 필요한 기존 대용량 화면은 인덱스에서 대상 키를 먼저 찾은 뒤 해당 행의 본문을 조회했고, 데이터가 적은 화면에는 단순한 조회를 유지했습니다.",
                validation:
            ...
            {
                number: "03",
                title: "PDF 변환 요청 상태가 저장되기 전에 도착한 완료 응답 재처리",
                constraint:
                    "외부 PDF 변환을 요청한 뒤 애플리케이션이 요청 상태를 DB에 저장하기 전에 완료 응답이 도착하면, 대상 요청을 찾지 못해 정상 변환 결과가 누락될 수 있었습니다.",
                decision:
                    "Spring Retry로 요청 상태를 짧은 간격으로 다시 조회하되 재시도 간격을 점차 늘리고 무작위 지연을 더했습니다. 여러 완료 응답이 동시에 도착해도 재조회 시점이 한꺼번에 몰리지 않도록 했습니다.",
                validation:
            ...
            {
                number: "04",
                title: "외부 API 응답 지연 중 DB 연결 장기 점유와 같은 작업의 중복 실행 방지",
                constraint:
    ...
    },
    {
        ...
        oneLine:
            "세 기관의 수용자 자료를 검증해 군교정 DB에 반영하고, WebSquare 상태 변경 요청의 위조를 차단했으며, 대용량 파일은 업무 서버를 거치지 않고 저장소로 전송했습니다.",
        status: {
            label: "공개 범위",
        ...
        problems: [
            ...
            {
                number: "02",
                title: "WebSquare 저장 및 변경 요청에 위조 방지 토큰(CSRF)을 포함",
                constraint:
            ...
            {
                number: "03",
                title: "대용량 파일을 업무 서버를 거치지 않고 저장소로 직접 업로드",
                constraint:
                    "대용량 파일 본문이 업무 서버를 거치면 요청마다 메모리와 디스크 및 네트워크 입출력을 사용하고, 동시에 업로드할 때 서버 부하가 커질 수 있었습니다. 기존 파일 저장 시스템은 유지해야 했습니다.",
                decision:
                    "화면에서 업무 서버에 업로드 권한과 파일 정보를 요청하고, 검증을 통과하면 Presigned URL을 발급했습니다. 브라우저는 이 URL로 기존 파일 저장 시스템에 직접 업로드하며 업무 서버는 파일 본문을 받거나 중계하지 않았습니다.",
                validation:
            ...
            },
            {
                ...
                decision:
                    "Jenkins 실행 이력에서 배치 실행 시각과 성공 및 실패 단계를 먼저 확인하고, 같은 시각의 JEUS 및 업무 서버 로그에서 요청과 예외를 찾았습니다. 이후 Tibero의 입력 데이터, 처리 상태와 결과 데이터를 비교해 배치 실행 전, 처리 중 또는 DB 반영 단계 중 어디에서 멈췄는지 판단했습니다. 필요한 경우 기관 담당자와 실제 송수신 시각도 대조했습니다.",
                validation:
