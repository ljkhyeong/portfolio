...
const projects = [
    {
        ...
        screenshots: [
            {
                ...
                label: "업무 흐름",
                caption: "인수인계 타임라인, 진행이 멈춘 바통과 최근 결정 기록 확인",
        ...
        architecture: {
            label: "서비스 구성과 담당 업무",
            title: "중앙 서비스(Core)는 조직 정보와 작업 공간 공유 키를 관리하고, 링크 생성, URL 점검, 이벤트 전달, 주간 보고서, 캘린더 구독과 WebRTC 스터디룸은 6개 서비스가 나눠 처리합니다.",
            description:
                "Core는 팀, 시즌, 역할, 반복 업무, 의사결정과 인수인계를 저장하고 작업 공간 공유 키를 확인합니다. 계정과 연결된 활동 중인 스터디 구성원 여부는 ROUND 참여권을 발급할 때 추가로 확인합니다. GO, WATCH, RELAY, BRIEF, CAL과 ROUND는 각각 별도 코드 저장소와 실행 환경으로 나눴습니다. DB가 필요한 서비스끼리 데이터를 공유하지 않으며, ROUND의 방과 연결 상태는 프로세스 메모리에 저장합니다.",
            tradeoff:
        ...
        services: [
            {
                ...
                summary:
                    "팀, 역할, 반복 업무, 의사결정과 인수인계를 저장하고 일반 조직 운영 요청의 작업 공간 공유 키를 확인합니다. ROUND 참여권 발급 때에는 계정과 연결된 활동 중인 스터디 구성원인지 추가로 확인합니다.",
                detail: "팀, 시즌, 역할, 운영 기록, 작업 공간 공유 키, 계정과 스터디 구성원 연결 및 ROUND 참여권 발급",
                evidence:
                    "준비 → 전달 → 수락 순서, 준비 및 전달 단계의 취소와 역할별 진행 중인 인수인계 1건 제약 테스트 · PRD 6 · ADR 19 · OpenAPI · 서비스 간 이벤트 및 참여권 규격",
                input: "팀 및 시즌 생성, 역할과 반복 업무 등록, 의사결정 기록, 바통 준비, 전달, 수락 및 취소와 ROUND 방 참여 요청",
                inputRule:
                    "일반 조직 운영 요청은 작업 공간 공유 키와 대상 데이터의 팀 및 시즌 소속을 확인합니다. 계정 세션과 계정에 연결된 활동 중인 스터디 구성원은 멤버십 관리와 ROUND 참여권 발급 요청에만 추가로 확인합니다.",
            ...
            },
            {
                ...
                role: "주간 운영 요약",
                summary:
                    "BRIEF는 Core 개발 브랜치가 생성하는 두 번째 이벤트 형식(v2)을 받아 운영 점검 목록을 만듭니다. 담당자가 없는 역할과 담당 종료가 임박했지만 후임자가 없는 역할을 찾습니다. 위험 요소가 기록된 역할에서는 책임 목록, 인수인계에 포함할 업무 항목과 역할 수행 자료가 빠졌는지, 끝나지 않은 인수인계 항목이 있는지 확인합니다. 반복해서 마감이 지난 업무와 담당 교대 시점이 가까운 미완료 인수인계도 목록에 포함합니다. 보고서 생성 요청을 받으면 해당 주간, BRIEF가 마지막으로 받은 이벤트 순번과 포함 항목을 함께 저장합니다. 이후 이벤트가 바뀌어도 이미 만든 보고서는 수정하지 않습니다.",
                contribution:
                ...
                evidence:
                    "실제 Core 실행 JAR와 BRIEF 실행 JAR는 JSON 계약 패키지 2.0.0-rc.1 기준의 로컬 HTTP로 연결해 검증했습니다. Caddy 내부 인증서 HTTPS 앞단에는 계약 예시 JSON만 전송해 인증과 수신을 별도로 검증했습니다.",
                ...
                status: "JSON 계약 패키지 2.0.0-rc.1 기준으로 실제 Core 실행 JAR와 BRIEF 실행 JAR를 로컬 HTTP에서 연결해 Core의 역할 및 인수인계 정보 변경이 BRIEF 운영 점검 목록에 반영되는지, 장애 재시도, 동일 이벤트 재전달과 현재 및 직전 Bearer 토큰을 함께 허용하는 교체 구간을 확인했습니다. Caddy 내부 인증서 HTTPS 앞단에는 실제 Core가 아닌 계약 예시 JSON을 전송해 인증과 수신만 별도로 확인했습니다. 입력 규칙을 강화한 현재 2.0.0-rc.2는 Core의 실제 JSON 생성 코드와 다시 검증해야 하며, 실제 Core에서 Caddy로 보내는 HTTPS 전달도 아직 검증하지 않았습니다. 최신 구현은 원격 개발 브랜치에 있으며 공개 main 반영 전이고 원격 스테이징에도 배포하지 않았습니다.",
                tradeoff:
            ...
            },
            {
                ...
                visibility: "비공개 저장소 / 설계와 테스트 요약 공개",
                status: "원격 개발 브랜치에서 Core의 실제 참여권 서명 코드로 만든 참여권을 ROUND가 로컬 HTTPS와 암호화된 WebSocket(WSS) 연결에서 검증한 뒤 방에 입장시키는 과정을 확인했습니다. 공인 DNS와 자동 발급 TLS 인증서, 외부 TURN 미디어 중계 서버(coturn)와 6명 장시간 접속 테스트는 아직 하지 않았습니다.",
                tradeoff:
        ...
        proofs: [
            {
                ...
                result: "준비 → 전달 → 수락 순서를 강제하고 준비 또는 전달 단계에서는 취소를 허용했으며, 역할별 진행 중인 인수인계를 1건으로 유지",
            ...
            },
            {
                ...
                method: "PostgreSQL 통합 테스트, 실제 Core 실행 JAR에서 BRIEF 실행 JAR로 보낸 로컬 HTTP 이벤트와 Caddy 내부 인증서 HTTPS 앞단에 보낸 계약 예시 JSON을 구분해 확인",
                ...
                scope: "실제 Core 실행 JAR와 BRIEF 실행 JAR 연결은 JSON 계약 패키지 2.0.0-rc.1 및 로컬 HTTP 기준 · Caddy HTTPS는 계약 예시 JSON만 검증 · 현재 2.0.0-rc.2의 실제 Core JSON, Core-Caddy HTTPS, 공개 main 및 원격 스테이징 반영 전 · 2026.08.27",
            },
            {
        ...
        oneLine:
            "Core는 조직 데이터, 작업 공간 공유 키와 ROUND 방 참여 가능 여부를 관리합니다. GO, WATCH, RELAY, BRIEF, CAL, ROUND는 각각 짧은 링크, URL 점검, 서비스 간 이벤트 전달, 주간 보고서, 캘린더 구독과 WebRTC 스터디룸을 처리합니다.",
        status: {
            label: "현재 상태",
            text: "Core와 6개 서비스의 주요 기능을 구현했습니다. CAL은 Core의 실제 일정 JSON 생성 코드로 만든 데이터를 컨테이너에서 처리했고, ROUND는 Core의 실제 참여권 서명 코드로 만든 참여권을 HTTPS 및 암호화된 WebSocket(WSS)에서 검증했습니다. BRIEF는 JSON 형식 2.0.0-rc.1 기준으로 실제 Core 실행 JAR에서 BRIEF 실행 JAR로 로컬 HTTP 이벤트를 보내 연동했고, Caddy HTTPS는 계약 예시 JSON으로 별도 확인했습니다. 현재 2.0.0-rc.2의 실제 Core JSON 재검증, 실제 Core-Caddy HTTPS 전달과 공개 main 반영이 남아 있습니다. 실제 운영 자격 증명과 외부 서비스를 사용하는 공개 환경의 전체 연동 검증도 아직 완료하지 않았습니다.",
        },
        visualCaption:
            "Core는 팀, 역할, 반복 업무, 의사결정, 인수인계와 작업 공간 공유 키를 관리합니다. GO, WATCH, RELAY, BRIEF, CAL과 ROUND는 각각 별도 코드 저장소와 실행 환경으로 나눴습니다. DB가 필요한 서비스끼리 데이터를 공유하지 않고, ROUND의 방과 연결 상태는 프로세스 메모리에 저장합니다.",
        problems: [
            {
                number: "01",
                ...
                decision:
                    "Core에는 조직, 역할, 인수인계와 작업 공간 공유 키를 두고 GO, WATCH, RELAY, BRIEF, CAL과 ROUND는 각각 별도 코드 저장소와 실행 환경으로 분리했습니다. DB가 필요한 서비스끼리 데이터를 공유하지 않으며 ROUND의 방과 연결 상태는 프로세스 메모리에 저장합니다. 다른 서비스에 알려야 하는 상태 변경은 업무 데이터와 함께 전송 대기 이벤트로 DB에 저장하고, 업무 저장이 끝난 뒤 전송합니다.",
                validation:
            ...
            },
            {
                ...
                decision:
                    "준비 단계에서 현재 담당자, 다음 담당자와 다음 담당 기간을 고정했습니다. 전달할 때 인수인계 업무 항목 수, 미완료 항목 수와 역할 자료 수를 저장하고, 업무 항목이 없거나 미완료 항목이 있거나 자료가 없으면 경고 확인을 요구했습니다. 준비 또는 전달 단계에서는 이전 담당자가 취소할 수 있게 했고, 취소하면 다음 담당자 예약을 해제했습니다. 전달 뒤에는 수락 또는 취소 전까지 역할, 인수인계 업무와 자료를 수정하지 못하게 했고, 수락할 때 담당자와 담당 기간을 한 트랜잭션에서 변경했습니다.",
                validation:
                    "역할별 열린 바통 1건 제약, 준비 → 전달 → 수락 순서와 준비 또는 전달 단계의 취소를 상태 전이 테스트로 확인했습니다. 중복 교대와 전달 후 변경도 차단했습니다.",
                boundary:
            ...
            },
            {
                ...
                validation:
                    "PostgreSQL 통합 테스트와 JSON 계약 패키지 2.0.0-rc.1을 사용해 실제 Core 실행 JAR에서 BRIEF 실행 JAR로 로컬 HTTP 이벤트를 보냈습니다. 같은 이벤트와 과거 개정 번호를 반영하지 않고, 건너뛴 최신 개정은 누락 구간을 기록한 뒤 반영하는지 확인했습니다. 현재 Bearer와 직전 Bearer를 잠시 함께 허용해 전송 중단 없이 토큰을 교체하는 과정도 확인했습니다. Caddy 내부 인증서 HTTPS 앞단의 인증과 수신은 계약 예시 JSON으로 별도 확인했습니다.",
                boundary:
                    "실제 Core 실행 JAR와 BRIEF 실행 JAR를 로컬 HTTP로 연결해 검증했고, Caddy 내부 인증서 HTTPS에는 계약 예시 JSON만 전송했습니다. 입력 규칙을 강화한 현재 2.0.0-rc.2는 Core의 실제 JSON 생성 코드와 다시 검증해야 합니다. 실제 Core에서 Caddy로 보내는 HTTPS 전달, 공개 main 반영과 공인 DNS를 사용한 원격 스테이징 전달도 아직 검증하지 않았습니다.",
            },
            {
                number: "10",
                serviceIds: ["brief"],
                ...
                decision:
                    "수락한 이벤트 전체를 다시 읽어 운영 점검 목록을 만들었습니다. 보고서를 만들 때 해당 주간과 BRIEF가 마지막으로 받은 이벤트 순번, 포함 항목을 함께 저장하고, 같은 주간과 같은 수신 순번 및 항목으로 다시 요청하면 기존 보고서를 반환했습니다. 한 번 생성한 보고서는 수정하지 않고 당시 상태를 확인할 수 있도록 보관합니다.",
                validation:
                ...
                boundary:
                    "Core 개발 브랜치가 생성하는 두 번째 이벤트 형식(v2)에는 담당자가 없는 역할, 담당 종료가 임박했지만 후임자가 없는 역할, 위험 요소가 기록된 역할 중 책임 목록이 없거나 인수인계에 포함할 업무 항목이 없거나 끝나지 않은 항목이 있거나 역할 수행 자료가 없는 역할, 반복해서 마감이 지난 업무와 담당 교대 시점이 가까운데 인수인계를 시작하지 않았거나 전달 및 수락 조건이 남은 경우가 포함됩니다. 이전 이벤트 형식(v1)도 계속 처리하므로 항목을 추가할 때 두 버전의 이벤트 JSON, 운영 점검 목록 생성 규칙과 주간 보고서 비교 규칙을 함께 변경해야 합니다. 입력 규칙을 강화한 JSON 계약 패키지 2.0.0-rc.2는 Core의 실제 JSON 생성 코드와 다시 검증해야 합니다.",
            },
            {
                number: "11",
                serviceIds: ["cal"],
    ...
    },
    {
        ...
        screenshots: [
            ...
            },
            {
                ...
                label: "클래스 선택",
                caption: "클래스 소개와 예약 가능한 수업 선택 안내",
        ...
        oneLine:
            "결제 및 환불 요청 ID로 중복 처리를 막았습니다. 미전송 알림은 DB에서 다시 처리했습니다. 동시 예약은 클래스와 예약 시간 행을 잠근 뒤 정원을 확인했고, 동시 주문은 재고 행을 잠근 뒤 수량을 차감했습니다.",
        status: {
            label: "운영 상태",
            text: "AWS 운영 환경에 배포했으나 상시 리소스 비용으로 운영을 종료했습니다. 현재 공개 URL은 없습니다. 최신 로컬 브랜치에는 공개 화면 서버 렌더링, 옵션 조합별 재고, 예약 캘린더와 클래스별 정원 및 배송조회를 구현했습니다. 취소 전 환불 안내와 미수령 주문의 관리자 예외 환불, 관리자 30분 예약 시간표도 구현했습니다. 홈 공지 및 이벤트, 이벤트 쿠폰, 카카오 로그인과 계정 연결까지 반영했습니다. 기준 커밋은 2026.08.27 b50a9ef0이며 아직 원격 개발 브랜치와 공개 main에는 반영하지 않았습니다.",
        },
        visualCaption:
        ...
        problems: [
            ...
            },
            {
                ...
                decision:
                    "택배사 코드, 표시명과 운송장 번호를 주문 이행 정보에 저장하고 외부 배송조회 등록 상태는 별도로 관리했습니다. 요청 제한(HTTP 429), 서버 오류(HTTP 5xx) 및 통신 실패는 배치가 DB 상태를 읽어 다시 요청합니다. 웹훅은 HMAC-SHA256 서명과 5분 이내 전송 시각을 확인한 뒤 배송 상태와 이동 이력만 갱신하며, 주문 완료는 관리자가 별도로 확정합니다.",
                validation:
    ...
    },
    {
        ...
        architecture: {
            label: "입력한 커밋과 커밋 종류별 규칙으로 확정한 비교 기준에 각각 저장된 코드만 사용",
        ...
        proofs: [
            {
                ...
                result: "전체 커밋 ID와 비교 기준을 고정하고, 로컬 Git의 외부 텍스트 변환 설정(textconv)과 색상 설정을 끈 상태에서도 파일 이름 변경과 변경 줄 수를 유지하며 UTF-8이 아닌 경로는 분석 전에 거절",
        ...
        oneLine:
            "입력한 로컬 커밋과 일반, 최초 및 병합 커밋별 규칙으로 확정한 비교 기준에 각각 저장된 코드를 대조해 리뷰를 새 HTML 파일로 저장하되, 현재 수정 중인 파일과 이전 대화는 분석에서 제외",
        status: {
            label: "공개 상태",
            text: "SeungIl 님이 개발한 Hope 3.0.3의 Git 이력, MIT 라이선스와 기존 기능을 유지한 비공식 포크입니다. 제가 추가한 Commit Diff와 관련 테스트 및 문서는 README와 NOTICE에 구분했습니다. 공개 main은 3.1.1입니다. 원격 개발 브랜치 4.0.0에서는 비공개 경로와 토큰 차단에 더해 Git의 외부 텍스트 변환 설정(textconv)과 색상 설정을 무시하고 UTF-8이 아닌 파일 경로를 거절하며, 중단된 리뷰 수집을 같은 실행에서 재개하도록 보완했습니다. 4.0.0은 아직 main과 태그에 반영하지 않았습니다.",
        },
        visualCaption:
        ...
        problems: [
            {
                number: "01",
                title: "현재 수정 파일을 제외하고 입력 커밋의 변경만 비교",
                constraint:
                    "스테이징한 파일, 수정 중인 파일과 추적하지 않는 파일을 함께 읽으면 특정 커밋에 없던 내용이 검토 결과에 섞일 수 있습니다.",
                decision:
                    "입력한 16진수 커밋 ID를 전체 ID로 확정했습니다. 일반 커밋은 첫 번째 부모, 최초 커밋은 빈 상태, 병합 커밋은 사용자가 고른 부모를 비교 기준으로 삼았습니다. 파일 본문과 변경 내역은 현재 수정 중인 파일이 아니라 확정한 커밋과 비교 기준에 저장된 코드에서 읽습니다. 4.0.0 개발 브랜치에서는 사용자의 Git 외부 텍스트 변환 설정(textconv)과 색상 출력을 끄고, 파일 경로를 바이트 단위로 확인해 UTF-8이 아니면 분석 전에 거절합니다.",
                validation:
    ...
    },
    {
        ...
        oneLine:
            "군사법원, 군검찰 및 군사경찰의 수용자 자료를 검증해 군교정 DB에 반영하고, WebSquare 상태 변경 요청의 위조를 차단했으며, 대용량 파일은 업무 서버를 거치지 않고 저장소로 전송했습니다.",
        status: {
            label: "공개 범위",
