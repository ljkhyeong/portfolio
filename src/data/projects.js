...
const projects = [
    {
        ...
        services: [
            ...
            },
            {
                ...
                status: "JSON 계약 패키지 2.0.0-rc.1 기준으로 실제 Core와 BRIEF 실행 JAR를 로컬 HTTP에서 연결해 원본 변경, 장애 재시도, 동일 이벤트 재전달과 현재 및 직전 Bearer 토큰을 함께 허용하는 교체 구간을 확인했습니다. 별도로 BRIEF의 Caddy 내부 인증서 HTTPS 앞단에는 계약 예시 JSON을 전송해 인증과 수신을 확인했습니다. 입력 규칙을 강화한 현재 2.0.0-rc.2는 Core의 실제 JSON 생성 코드와 다시 검증해야 하며, 실제 Core에서 Caddy로 보내는 HTTPS 전달도 아직 검증하지 않았습니다. 최신 구현은 공개 main 반영 전이며 원격 스테이징에도 배포하지 않았습니다.",
                tradeoff:
                    "각 항목이 선택된 이유를 이벤트 종류와 심각도로 확인할 수 있습니다. BRIEF는 현재 업무 상태 전체를 담는 두 번째 이벤트 형식(v2)과 이전 형식(v1)을 함께 처리합니다. 이벤트 종류가 추가되면 Core가 보내는 JSON 형식, BRIEF의 운영 점검 목록 생성 규칙과 주간 보고서 비교 규칙을 두 버전에 맞춰 함께 변경해야 합니다.",
                repository: {
                    href: "https://github.com/ljkhyeong/baton-brief",
                    label: "BRIEF 공개 저장소",
                    note: "공개 main에는 초기 프로젝트 뼈대만 있으며 최신 구현은 로컬 개발 브랜치에 있습니다.",
                },
                documentation: [
                    { label: "PRD", count: "22" },
                    { label: "ADR", count: "5" },
            ...
            },
            {
                ...
                database: "PostgreSQL",
                visibility: "공개 저장소 / 일정 JSON 형식 1.0.0",
                status: "시즌 일정의 수신, 변경, 취소와 읽기 전용 iCalendar 구독 기능을 구현했습니다. Core의 실제 일정 JSON 생성 코드로 만든 데이터를 CAL 컨테이너에 보내 정상 반영되는 것도 확인했습니다. 운영 자격 증명을 사용한 실제 이벤트 전송과 공개 캘린더 앱 구독은 아직 하지 않았습니다.",
                tradeoff:
                    "읽기 전용 구독은 외부 캘린더에서 쉽게 사용할 수 있지만, 비동기 반영 지연과 캘린더 앱별 동작 차이를 관리해야 합니다.",
                repository: {
                    href: "https://github.com/ljkhyeong/baton-cal",
                    label: "CAL 공개 저장소",
                    note: "일정 JSON 형식 1.0.0과 Core가 실제로 만든 일정 데이터의 호환성 테스트를 공개합니다.",
                },
                documentation: [
                    { label: "PRD", count: "2" },
                    { label: "ADR", count: "2" },
            ...
            },
            {
                ...
                visibility: "비공개 저장소 / 설계와 테스트 요약 공개",
                status: "원격 개발 브랜치에서 Core의 실제 참여권 서명 코드로 만든 참여권을 ROUND가 로컬 HTTPS와 WSS에서 검증하고 방에 입장시키는 과정을 확인했습니다. 공인 DNS와 자동 발급 TLS 인증서, 외부 coturn 미디어 중계와 6명 장시간 접속 테스트는 아직 하지 않았습니다.",
                tradeoff:
        ...
        proofs: [
            ...
            },
            {
                ...
                method: "Core의 실제 참여권 서명 코드로 만든 RS256 참여권을 ROUND 실행 JAR에 HTTPS 및 WSS로 전달하고 WebSocket 입장 테스트 실행",
        ...
        status: {
            label: "현재 상태",
            text: "Core와 6개 서비스의 주요 기능을 구현했습니다. CAL은 Core의 실제 일정 JSON 생성 코드로 만든 데이터를 컨테이너에서 처리했고, ROUND는 Core의 실제 참여권 서명 코드로 만든 참여권을 HTTPS 및 WSS에서 검증했습니다. BRIEF는 JSON 형식 2.0.0-rc.1 기준으로 실제 Core 실행 JAR와 로컬 HTTP에서 연동했고, Caddy HTTPS는 계약 예시 JSON으로 별도 확인했습니다. 현재 2.0.0-rc.2의 실제 Core JSON 재검증, 실제 Core-Caddy HTTPS 전달과 공개 main 반영이 남아 있습니다. 실제 운영 자격 증명과 외부 서비스를 사용하는 공개 환경의 전체 연동 검증도 아직 완료하지 않았습니다.",
        },
        visualCaption:
        ...
        problems: [
            ...
            },
            {
                ...
                decision:
                    "준비할 때 현재 및 다음 담당자와 다음 담당 기간을 고정하고, 전달할 때 활성 및 미완료 바통 항목 수와 역할 자료 수를 저장했습니다. 전달 후에는 수락 또는 취소 전까지 역할, 바통 항목과 자료를 수정하지 못하게 했고, 수락 시 담당자와 담당 기간을 한 트랜잭션에서 변경했습니다.",
                validation:
            ...
            },
            {
                ...
                boundary:
                    "실제 Core-BRIEF 실행 JAR 교차 검증은 로컬 HTTP로 수행했고, Caddy 내부 인증서 HTTPS에는 계약 예시 JSON만 전송했습니다. 입력 규칙을 강화한 현재 2.0.0-rc.2는 Core의 실제 JSON 생성 코드와 다시 검증해야 합니다. 실제 Core에서 Caddy로 보내는 HTTPS 전달, 공개 main 반영과 공인 DNS를 사용한 원격 스테이징 전달도 아직 검증하지 않았습니다.",
                print: {
            ...
            },
            {
                ...
                boundary:
                    "Core 개발 브랜치가 생성하는 두 번째 이벤트 형식(v2)에는 담당자가 없는 역할, 담당 종료가 임박했지만 후임자가 없는 역할, 위험 요소가 기록됐지만 책임 목록, 인수인계할 업무 항목 또는 역할 수행에 필요한 자료가 부족한 역할, 반복해서 마감이 지난 업무와 담당 교대 시점이 가까운데 인수인계를 시작하지 않았거나 전달 및 수락 조건이 남은 경우가 포함됩니다. 이전 이벤트 형식(v1)도 계속 처리하므로 항목을 추가할 때 두 버전의 이벤트 JSON, 운영 점검 목록 생성 규칙과 주간 보고서 비교 규칙을 함께 변경해야 합니다. 입력 규칙을 강화한 JSON 계약 패키지 2.0.0-rc.2는 Core의 실제 JSON 생성 코드와 다시 검증해야 합니다.",
            },
            {
                number: "11",
                serviceIds: ["cal"],
            ...
            },
            {
                ...
                boundary:
                    "스터디 구성원 자격을 회수해도 다음 참여권 갱신 또는 기존 참여권 만료까지 반영이 늦어질 수 있습니다. 공인 DNS와 인증서, 외부 coturn 미디어 중계, 실기기 Safari와 6명 장시간 접속 테스트는 남아 있습니다.",
                print: {
                    label: "ROUND / 방 입장 권한",
                    problem:
    ...
    },
    {
        ...
        evidenceAsOf: "2026.08.27 로컬 커밋 1e1e7a87 기준 · 공개 main 및 원격 개발 브랜치 미반영",
        ...
        status: {
            label: "운영 상태",
            text: "AWS 운영 환경에 배포했으나 상시 리소스 비용으로 운영을 종료했습니다. 현재 공개 URL은 없습니다. 공개 화면 서버 렌더링, 옵션 조합별 재고와 예약 캘린더 구현은 2026.08.27 로컬 커밋 1e1e7a87 기준이며 아직 원격 개발 브랜치와 공개 main에는 반영하지 않았습니다.",
        },
        visualCaption:
        ...
        problems: [
            ...
            },
            {
                ...
                decision:
                    "예약은 클래스 행을 먼저 잠근 뒤 예약 시간 행을 ID 순서로 잠갔습니다. 일반 상품 재고는 상품 ID, 옵션별 재고는 SKU ID 오름차순으로 잠그고 수량 확인과 차감을 한 트랜잭션에서 처리했습니다.",
                validation:
    ...
    },
    {
        ...
        architecture: {
            label: "입력한 커밋과 비교 대상으로 확정한 부모 커밋의 코드만 사용",
            title: "현재 수정 중인 파일을 제외하고, 사용자가 입력한 커밋과 비교 대상으로 확정한 부모 커밋의 변경만 검토합니다.",
            description:
        ...
        proofs: [
            ...
            },
            {
                ...
                rule: "입력 커밋과 부모를 확정하고, 리뷰 설명의 파일과 줄 및 정해진 JSON 형식을 확인한 뒤 저장 직전 커밋을 다시 확인한 경우에만 새 HTML 파일 생성",
        ...
        oneLine:
            "입력한 로컬 커밋을 비교 대상으로 확정한 부모 커밋과 대조해 변경 코드와 리뷰를 새 HTML 파일로 저장하되, 현재 수정 중인 파일과 이전 대화는 분석에서 제외",
        status: {
            label: "공개 상태",
            text: "SeungIl 님이 개발한 Hope 3.0.3의 Git 이력, MIT 라이선스와 기존 기능을 유지한 비공식 포크입니다. 제가 추가한 Commit Diff와 관련 테스트 및 문서는 README와 NOTICE에 구분했습니다. 공개 main은 3.1.1이고, 원격 개발 브랜치에서는 Commit Diff 실행 명령과 비공개 경로 및 토큰 차단 규칙을 보완한 4.0.0을 검증 중입니다. 4.0.0은 아직 main과 태그에 반영하지 않았습니다.",
        },
        visualCaption:
            "입력 커밋과 비교 대상 부모 커밋을 먼저 확정하고 현재 수정 파일을 제외한 변경 코드를 수집한 뒤, 리뷰 설명이 실제 파일과 줄을 가리키는지 확인한 결과만 새 HTML로 저장합니다.",
        problems: [
            {
                number: "01",
                ...
                decision:
                    "입력한 16진수 커밋 ID를 전체 객체 ID로 확정하고 선택한 부모와 비교했습니다. 파일 본문과 변경 내역은 현재 수정 중인 파일이 아니라 확정한 커밋과 부모에 저장된 코드에서 읽습니다.",
                validation:
            ...
            },
            {
                ...
                validation:
                    "검증 전후 대상 커밋을 바꾸고 기존 출력 경로, 심볼릭 링크와 동시 저장을 재현해, 기존 HTML과 다른 실행의 작업 디렉터리를 바꾸지 않고 저장을 중단하는지 확인했습니다.",
                boundary:
        ...
        proofs: [
            {
                ...
                method: "기관별 배치 실행 결과, 업무 서버(WAS) 로그와 Tibero 처리 상태 확인",
        ...
        problems: [
            {
                ...
                validation:
                    "기관별 자료 수신 여부와 Jenkins 실행 결과, 업무 서버(WAS) 로그 및 Tibero 처리 상태를 대조해 중단 단계를 찾고, 재처리 후 인적정보와 영장정보가 군교정 DB에 반영되는 것까지 확인했습니다.",
                boundary:
            ...
            },
            {
                ...
                constraint:
                    "대용량 파일 본문이 업무 서버(WAS)를 거치면 요청마다 메모리와 I/O를 사용하고, 동시에 업로드할 때 서버 부하가 커질 수 있었습니다. 기존 파일 저장 시스템은 유지해야 했습니다.",
                decision:
