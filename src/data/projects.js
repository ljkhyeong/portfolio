...
const projects = [
    ...
    },
    {
        ...
        evidenceTitle: "주요 구현 및 확인 결과",
        systemTitle: "군사법원, 군검찰 및 군사경찰 데이터 연계 흐름",
        systemNavLabel: "연계 흐름",
        proofs: [
            {
                item: "군사법원, 군검찰 및 군사경찰 데이터 연계",
                method: "Jenkins 실행 결과, WAS 로그와 Tibero 처리 상태 확인",
                rule: "세 기관의 입력 수신, 형식 검증과 DB 반영 단계를 기관별 배치로 실행하고 각 단계의 처리 결과를 확인",
                result: "오류가 발생한 처리 단계와 미반영 데이터를 식별하고 해당 기관 데이터의 재처리 결과까지 확인",
                scope: "운영 건수, 내부 데이터와 세부 연계 규격은 비공개",
            },
            {
                item: "요청 위조 방지",
                method: "정상 및 차단 요청 시나리오 확인",
                rule: "Spring Security가 생성한 CSRF 토큰을 WebSquare 화면 스펙에 맞춰 전달하고 기존 업무 요청에 포함",
                result: "WebSquare의 정상 요청은 처리하고 토큰이 없거나 일치하지 않는 요청은 Spring Security 필터 단계에서 차단",
                scope: "폐쇄망 환경에서 확인",
            },
            {
                item: "업로드 파일 형식 검사",
                method: "정상 파일과 확장자를 바꾼 파일 업로드 확인",
                rule: "화면에서 기존 파일 업로드 솔루션을 바로 호출하지 않고 WAS 검증을 거쳐 호출하도록 순서를 변경",
                result: "WAS에서 확장자와 Apache Tika가 확인한 실제 형식을 비교하고 허용된 파일만 기존 파일 솔루션에 전달",
                scope: "폐쇄망 환경에서 확인",
            },
        ],
        category: "BEINTECH / 국방부 SI",
        role: "군교정 기능 개발, 군사법원, 군검찰 및 군사경찰 데이터 연계와 운영 대응",
        oneLine:
            "기관별 데이터 연계 배치와 WebSquare 보안 연동, WAS 로그 및 DB 상태 기반 장애 분석",
        status: {
            label: "공개 범위",
            text: "BEINTECH 소속으로 수행한 보안이 필요한 국방부 SI입니다. 운영 데이터, 접속 환경과 세부 연계 규격은 공개하지 않고 직접 수행한 개발과 운영 업무만 정리했습니다.",
        },
        visualCaption:
            "군사법원, 군검찰 및 군사경찰의 연계 데이터를 기관별 배치에서 검증하고 군교정 업무 DB에 반영하는 흐름입니다.",
        problems: [
            {
                number: "01",
                title: "군사법원, 군검찰 및 군사경찰 데이터 연계 배치 구현",
                constraint:
                    "군사법원, 군검찰과 군사경찰마다 입력 데이터 형식과 실행 시간이 달랐고, 연계가 중단되면 해당 데이터를 사용하는 군교정 업무도 처리할 수 없었습니다.",
                decision:
                    "세 기관의 입력 조건에 맞춰 데이터 수신, 형식 검증과 DB 반영 단계를 기관별 배치로 구현했습니다. Jenkins에서 기관별 실행 이력과 실패 단계를 확인하고 필요한 배치만 재처리하도록 구성했습니다.",
                validation:
                    "기관별 입력 수신 여부, Jenkins 실행 결과, WAS 로그와 Tibero 처리 상태를 확인해 중단 단계를 찾고 재처리 후 데이터 반영까지 확인했습니다.",
                boundary:
            ...
            },
            {
                ...
                decision:
                    "Spring Security가 생성한 CSRF 토큰을 WebSquare 화면 스펙에 맞춰 전달해 기존 업무 요청에 포함되도록 연결했습니다. 파일 업로드는 기존 파일 솔루션 로직을 수정하지 않고 화면에서 WAS 검증을 먼저 호출한 뒤 검증을 통과한 파일만 기존 파일 솔루션으로 전달하도록 순서를 변경했습니다. WAS에서는 확장자와 Apache Tika가 확인한 실제 형식을 함께 비교했습니다.",
                validation:
                    "정상 CSRF 토큰과 누락 및 불일치 토큰 요청, 정상 파일과 확장자를 바꾼 파일을 각각 실행해 CSRF 요청 차단 여부와 기존 파일 솔루션 전달 여부를 확인했습니다.",
                boundary:
            ...
            },
            {
                ...
                constraint:
                    "통합 APM이나 분산 추적 도구를 추가하기 어려운 폐쇄망 레거시 환경이었고, 화면 오류의 원인이 요청 처리, SQL, 기관 연계 배치 또는 DB 반영 중 어디에 있는지 바로 알기 어려웠습니다.",
                decision:
