...
BEINTECH에서 수행한 전자영장 및 군사법 기관 연계 업무와, 결제 및 알림 장애 처리를 구현한 happyGallery, 조직 운영 기능을 Core와 6개 서비스로 나눈 BATON을 소개하는 React 포트폴리오입니다.

## 경력 프로젝트

-   **전송형 전자영장 시스템**: BEINTECH 소속으로 LG CNS 컨소시엄에 참여해 독립망 간 기관 연계 인터페이스와 Spring Batch를 개발한 경력 사례
-   **차세대 군사법 정보 시스템**: 수용자 인적정보 및 영장정보를 군교정 DB에 반영하는 배치를 개발하고, 상태 변경 요청에는 CSRF 토큰을 적용해 위조 요청을 차단했으며, 대용량 파일은 WAS를 거치지 않고 저장소에 직접 업로드한 경력 사례

## 개인 프로젝트

-   **BATON**: Core가 조직, 역할, 인수인계와 접근 권한을 관리하고, 짧은 링크, URL 점검, 서비스 간 이벤트 전달, 주간 보고서, 캘린더 구독과 WebRTC 스터디룸을 6개 서비스로 나눠 구현한 프로젝트
-   **happyGallery**: 결제사 응답을 받지 못하면 요청 ID로 기존 승인 및 환불 결과를 조회하고, 보내지 못한 알림은 DB에서 다시 전송하며, 옵션별 재고는 잠금 순서를 고정해 초과 차감을 막은 서비스

## 오픈소스 및 개발 도구

-   **Hope Commit**: SeungIl 님이 개발한 Hope 3.0.3을 포크한 비공식 프로젝트. 입력한 커밋과 부모 커밋에 저장된 코드만 비교하고, 리뷰 설명이 실제 변경 파일과 줄을 가리키는지 확인한 뒤 새 HTML 파일로 저장하는 Commit Diff를 추가

## 교육 프로젝트

-   **WebRTC/HLS 현장강의 보조 서비스**: 2023년 6인 팀에서 RTP 영상을 HLS로 변환하는 서버와 실시간 및 다시보기 React 화면을 구현하고, HLS 재생 지연을 약 35초에서 17초로 줄인 교육 프로젝트
...
`/search`에서는 프로젝트 개요, 구현 구조와 선택 이유, 문제 해결 방법과 공개한 대표 문서를 검색할 수
