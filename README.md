...
공공 SI의 기관 연계 인터페이스와 Spring Batch, 결제 및 알림 실패 복구를 구현한 모놀리식 애플리케이션, Core와 6개 마이크로서비스로 구성한 BATON을 정리한 React 포트폴리오입니다.
...
-   **BATON**: Core가 조직, 역할, 인수인계와 접근 권한을 관리하고, 짧은 링크, URL 점검, 알림 전송, 주간 보고서, 캘린더 구독과 WebRTC 스터디룸을 6개 서비스로 나눠 구현한 프로젝트
-   **happyGallery**: 공방 작품 주문과 클래스 예약을 제공하고, 결제 결과 미수신은 기존 처리 상태 조회, 미전송 알림은 DB 재조회, 옵션별 재고는 고정된 잠금 순서로 처리한 Spring Boot 및 React 서비스
