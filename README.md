...
BEINTECH에서 Java와 Spring으로 기관 연계 서버 및 배치를 개발하고, 개인 프로젝트에서 중복 처리 방지와 장애 복구를 구현한 백엔드 개발자 임정규의 포트폴리오입니다.

## 경력 프로젝트

-   **전송형 전자영장 시스템**: BEINTECH 소속으로 LG CNS 컨소시엄에 참여해 KICS 요청을 통신사와 집행포털 규격으로 변환해 전달하고, 제출 자료를 KICS에 반영하는 서버와 Spring Batch를 개발 중입니다.
-   **차세대 군사법 정보 시스템**: 군사법원, 군검찰 및 군사경찰의 수용자 자료를 검증해 군교정 DB에 반영하는 배치를 개발했습니다. CSRF 차단과 대용량 파일의 저장소 직접 업로드도 구현했습니다.

## 개인 프로젝트

-   **BATON**: Core가 조직, 역할과 인수인계를 관리합니다. 짧은 링크, URL 점검, 이벤트 전달, 주간 보고서, 캘린더 구독과 WebRTC 스터디룸은 6개 서비스로 분리했습니다.
-   **happyGallery**: 결제 및 환불 요청 ID로 중복 처리를 막고, 보내지 못한 알림을 다시 전송합니다. DB 잠금으로 예약 정원과 재고 초과를 방지했으며 AWS에서 운영한 이력이 있습니다.

## 오픈소스 및 개발 도구

-   **Hope Commit**: SeungIl 님이 개발한 Hope 3.0.3의 비공식 포크입니다. 일반 커밋은 첫 부모, 최초 커밋은 빈 상태, 병합 커밋은 사용자가 고른 부모와 비교하며 실제 변경 줄이 근거인 결과만 HTML로 저장하는 Commit Diff를 추가했습니다. 원본 개발자는 이 포크를 보증하거나 유지보수하지 않습니다.
