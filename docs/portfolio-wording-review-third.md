...
추가 수정안 8개와 이전 수정에서 도식에 남은 표현 1곳을 사용자 승인 후 모두 반영했습니다. 도식과 본문의 동작 설명, 성과 측정 기준, 대상이 불분명한 용어를 정리하고 검색 데이터, 공유 이미지와 PDF를 갱신했습니다. 아래 원문과 위치는 검토 당시 기준이며, 도식의 최종 제목과 배치는 실제 처리 흐름에 맞게 조정했습니다.
...
- 문구 검토에서는 소스의 화면 표시 경로와 설명을 읽고 대조했습니다.
- IntentTrace와 청년정책메이트의 관련 구현은 읽기만 했으며, 위 근거 파일에 미커밋 변경이 없는 것을 확인했습니다. 다른 저장소의 신규 기능이나 공개 상태를 포트폴리오에 추가하는 검토는 하지 않았습니다.
- IntentTrace 도식은 공개 요청 검증과 기존 기록 대체를 두 영역으로 구분했습니다. 청년정책메이트 도식은 일반 입력 화면과 개발용 서버 계산을 분리했습니다. 두 도식의 제목, 본문, 하단 설명과 대체 설명을 함께 수정했습니다.
- 기존 테스트의 변경 전 문구를 갱신했으며 `npm test`에서 25개 파일의 테스트 163개가 통과했습니다.
- 변경 관련 9개 경로를 1440px와 390px 너비로 확인했습니다. 두 도식의 문구 잘림, 페이지 가로 넘침과 브라우저 오류가 없었고, 모바일 도식의 가로 스크롤과 CAL 토큰 교체 문구를 확인했습니다.
- 공유 이미지와 PDF를 다시 생성했습니다. HLS 및 군사법 공유 이미지를 확인했고, PDF 12페이지와 교육 이력의 팀 시연 조건 및 HLS 지연 수치를 확인했습니다.
- `npm run build`가 통과했습니다. 검색 데이터, 공유 이미지 및 PDF 검사와 17개 경로의 메타데이터 생성이 완료됐습니다.
