...
자료 상태는 `upToDate`의 참·거짓 자료형, 문서 수의 정수 여부·상호 일치, 현재 자료 버전을 검사합니다. 잘못된 상태 응답에서는 동기화나 평가를 시작하지 않습니다. 상태 확인은 30초, 동기화는 10분, 검색·답변 평가는 요청당 2분으로 제한하며 자동 재시도하지 않습니다. 동기화 응답을 받지 못했다면 상태를 확인한 뒤 다시 실행합니다.
