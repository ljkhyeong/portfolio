...
`429`·`503` 응답에 유효한 `Retry-After`가 있으면 화면과 평가 도구에 대기 시간을 안내합니다. 초 단위 정수와 표준 HTTP 날짜를 읽으며, 값이 없거나 잘못되면 기존 오류 안내를 사용합니다. 응답 시점의 안내일 뿐 카운트다운이나 재시도 버튼 잠금은 아닙니다. 별도 프록시가 이 헤더를 보내는 경우에도 브라우저에서 읽을 수 있도록 CORS에 노출해야 합니다.
...
웹은 `503`의 `HUMAN_VERIFICATION_UNAVAILABLE` 코드를 확인해 자동 요청 방지 검증 장애를 별도로 안내합니다. 일반 검색·답변 서버 오류와 구분하며, 검색 결과는 유지하고 `Retry-After`가 있으면 대기 시간을 표시합니다.
...
도구의 오류 안내에는 실패한 API 경로를 표시합니다. 연결 실패, 본문 수신 실패, 시간 초과와 잘못된 JSON을 구분하며, 응답 본문을 받는 시간도 요청 제한에 포함합니다. 오류 본문 정리에 실패해도 먼저 받은 HTTP 상태와 대기 안내를 유지합니다.

자료 상태는 `upToDate`의 참·거짓 자료형, 문서 수의 정수 여부·상호 일치, 현재 자료 버전을 검사합니다. 잘못된 상태 응답에서는 동기화나 평가를 시작하지 않습니다. 상태 확인은 30초, 동기화는 10분으로 제한합니다. 검색·답변 평가는 웹과 같은 설정을 사용해 요청당 각각 90초·180초로 제한하며 자동 재시도하지 않습니다. 동기화 응답을 받지 못했다면 상태를 확인한 뒤 다시 실행합니다.

`scripts/knowledge-evaluation-cases.json`은 대표 질문 24개와 답할 근거가 없는 질문 4개입니다. 기본 평가는 검색 결과의 상위 5건 적중률과 MRR@5, 요청 시간을 기록하며 적중률이 85% 미만이면 실패합니다. OpenAI 프로필의 검색은 질문 임베딩 비용이 발생할 수 있습니다. RRF로 청크 순위를 결합한 뒤 문서별 최상위 문단 하나를 선택하므로 긴 문서의 여러 문단이 검색 결과를 중복 차지하지 않습니다. 답변은 선택한 문서의 추가 검색 문단을 함께 사용합니다. 문단 전달과 인용 연결은 회귀 테스트로 확인하며, 여러 절을 종합하는 답변의 의미 정확도는 실제 모델로 별도 평가합니다.

`KNOWLEDGE_SYNC_KEY`가 있으면 평가 전후에 자료 버전·문서 수·색인 일치를 확인합니다. 종료 상태가 다르거나 확인에 실패하면 새 보고서를 저장하지 않습니다. 키가 없으면 상태 확인을 생략하고 보고서의 `verifiedSourceRevision`을 `null`로 남깁니다. 전후 확인만으로 평가 중간의 변경·원복까지 감지할 수는 없으므로 평가 중에는 재배포와 동기화를 피합니다.
